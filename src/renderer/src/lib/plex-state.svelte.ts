...
import { applyTheme, getTheme, loadStoredThemeId, storeThemeId } from './themes'
...
import { GenreIndexController } from './genre-index-controller.svelte'
...
import { normalizeGenre } from '../../../shared/genre-text'
...
class PlexState {
  ...
  // MusicBrainz genre index (per-album genre map + indexer progress). Plex's
  // own genre tags are too coarse to browse — see genre-index.ts in main for
  // the numbers — so this backfills MB's, falling back to Last.fm tags. Read
  // via plexState.genreIndex.genresFor(album), never album.genres directly.
  genreIndex = new GenreIndexController({
    ensureAllAlbums: () => this.ensureAllAlbums(),
    clearGenreFilters: () => this.filterState.clearGenres(),
    serverId: () => (this.session?.signedIn ? this.session.preferredServerId : null)
  })
  private detachGenreProgress: (() => void) | null = null
  ...
  async init(): Promise<void> {
    ...
    this.detachGenreProgress = this.genreIndex.subscribe()
    this.detachWarmerProgress = window.cratedigger.plex.onWarmerProgress((p) => {
      const was = this.warmerProgress
      this.warmerProgress = p
      // A run that actually fetched something and reached the end of the
      // library is worth a word; silent resumes that only confirm cache are not.
      if (was.running && !p.running && p.total > 0 && p.done >= p.total && p.fetched > 0) {
        this.flashToast(
          'ok',
          p.errors > 0
            ? `Credits cached for the library — ${p.errors} album${p.errors === 1 ? '' : 's'} to retry next time`
            : `Credits cached for all ${p.total.toLocaleString()} albums`,
          6000
        )
      }
    })
    this.scheduleAutoWarm()
    ...
    // Re-send on launch so a kiosk that was rebuilt, restarted or offline
    // when the theme last changed still catches up.
    void this.pushThemeToKiosk()
    void this.labelIndex.loadLabels()
    void this.genreIndex.loadGenres()
  ...
  dispose(): void {
    ...
    this.detachGenreProgress?.()
    this.detachGenreProgress = null
    this.detachWarmerProgress?.()
    this.detachWarmerProgress = null
    if (this.#autoWarmTimer) clearTimeout(this.#autoWarmTimer)
    this.#autoWarmTimer = null
  ...
  private currentFilters(): AlbumFilters {
    return this.filterState.toAlbumFilters(!this.genreIndex.active)
  }
  ...
  private _afterFilterChange(): void {
    // Genre includes are normally the server's job, but once the MB genre
    // index has data they're ours — so they need the full library too.
    const genreIsClientSide =
      this.genreIndex.active && this.filterState.genreInclude.size > 0
    if (this.filterState.needsAllAlbums() || genreIsClientSide) {
      void this.ensureAllAlbumsLoaded()
    }
  ...
  // Session memo of album → resolved artist MBID, so repeat clicks on the
  // same artist name never re-pay the tagger/MB resolution below.
  #artistMbidByAlbum = new Map<string, string>()

  async openArtistFromAlbum(detail: AlbumDetail): Promise<void> {
    const name = detail.album.artist
    if (!name) return

    // Zero-network fast path: fresh credits carry the release's credited
    // artist MBID directly. Legacy cache entries predate the field — they
    // fall through to the resolution below.
    const direct = this.openAlbumLoader.credits?.albumArtistMbid
    if (direct) {
      await this.openEntity(direct, name)
      return
    }
    const memo = this.#artistMbidByAlbum.get(detail.album.ratingKey)
    if (memo) {
      await this.openEntity(memo, name)
      return
    }
    ...
    for (const mbid of candidateMbids) {
      try {
        // Hot path: fail fast on a wedged tagger and fall through to name search.
        const release = await window.cratedigger.tagger.getRelease({ mbid, timeoutMs: 5000 })
    ...
    this.#artistMbidByAlbum.set(detail.album.ratingKey, artistMbid)
  ...
  // Open an artist page from a name (chart cards, search chips, show
  // listings) with the page appearing on the click, not after the lookup.
  // The MBID resolves in tiers, cheapest first: the artist's own Plex
  // metadata when we have its rating key (one LAN call), then the cached
  // library artist list, then a MusicBrainz name search — which is paced at
  // ~1/s behind everything else in the MB queue and used to be the entire
  // wait, with nothing on screen changing until it returned.
  #artistOpenSeq = 0
  async openArtistByName(name: string, opts: { plexRatingKey?: string | null } = {}): Promise<void> {
    if (!this.session?.signedIn || !this.session.preferredServerId) return
    const serverId = this.session.preferredServerId
    const seq = ++this.#artistOpenSeq
    this.entityPage.beginResolving(name)
    const stillWanted = (): boolean =>
      seq === this.#artistOpenSeq && this.entityPage.resolving && this.entityPage.entity?.name === name
    let mbid: string | null = null
    try {
      if (opts.plexRatingKey) {
        mbid = await window.cratedigger.plex
          .getArtistMbid({ serverId, ratingKey: opts.plexRatingKey })
          .catch(() => null)
      }
      if (!mbid && stillWanted()) mbid = await this.libraryArtistMbidByName(name)
      if (!mbid && stillWanted()) mbid = await window.cratedigger.plex.searchArtistByName({ name })
    } catch (err) {
      if (!stillWanted()) return
      this.entityPage.cancelResolving()
      this.flashToast('err', err instanceof Error ? err.message : String(err))
      return
    }
    if (!stillWanted()) return
  ...
  openSearchArtist(name: string): Promise<void> {
    return this.openArtistByName(name)
  }

  // name → MBID over the library's artists, from one listing per session.
  // ~1,900 rows on the LAN — cheap once, instant after.
  #libraryArtistsByName: Promise<Map<string, string>> | null = null
  #libraryArtistsKey: string | null = null
  private libraryArtistMbidByName(name: string): Promise<string | null> {
    if (!this.session?.signedIn || !this.session.preferredServerId) return Promise.resolve(null)
    const serverId = this.session.preferredServerId
    if (!this.#libraryArtistsByName || this.#libraryArtistsKey !== serverId) {
      this.#libraryArtistsKey = serverId
      this.#libraryArtistsByName = window.cratedigger.plex
        .listLibraryArtistsWithGuids({ serverId })
        .then((rows) => {
          const map = new Map<string, string>()
          for (const r of rows) if (r.mbid && !map.has(r.name.toLowerCase())) map.set(r.name.toLowerCase(), r.mbid)
          return map
        })
        .catch(() => {
          this.#libraryArtistsByName = null
          return new Map<string, string>()
        })
    }
    return this.#libraryArtistsByName.then((m) => m.get(name.trim().toLowerCase()) ?? null)
  }
  ...
  /** Dress the living-room Pi in the same theme. The kiosk is a separate app
   *  on a separate box (nowplaying-kiosk), so it only gets the palette — see
   *  main/kiosk-theme.ts. Silent by design: no kiosk configured, or one that's
   *  asleep, must never interrupt switching themes here. */
  async pushThemeToKiosk(): Promise<void> {
    const theme = getTheme(this.themeId)
    try {
      await window.cratedigger.kiosk.pushTheme({
        id: theme.id,
        name: theme.name,
        scheme: theme.scheme,
        // $state proxies aren't structured-cloneable across IPC, and this
        // object is read straight out of a module constant — spread anyway so
        // a future themeId-derived source can't reintroduce that bug.
        tokens: { ...theme.tokens }
      })
    } catch {
  ...
  // The genre page. Matches against the same genres the album page shows —
  // the MusicBrainz / Last.fm index with Plex as fallback — plus whatever Plex
  // tags the listing carried. The listing alone was never enough: Plex's
  // section listing truncates each album to its first two genres, so an
  // album's own page could show "Pub Rock" while this filter, reading only
  // the listing, found nothing.
  filterLibraryByGenre(name: string): PlexAlbum[] {
    const target = normalizeGenre(name)
    if (!target) return []
    const source = this.allAlbums ?? this.albums
    const out = source.filter(
      (a) =>
        this.genreIndex.genresFor(a).some((g) => normalizeGenre(g) === target) ||
        a.genres.some((g) => normalizeGenre(g) === target)
    )
    out.sort((a, b) => (b.year ?? 0) - (a.year ?? 0))
    return out
  }
  ...
  // "Keep every album's credits on this Mac": start the warmer (credits only)
  // a minute after launch — after the library has painted and the first
  // clicks have had the MusicBrainz queue to themselves — and re-run every
  // few hours so newly imported albums get picked up. Already-cached albums
  // skip in milliseconds, so a re-run on a warmed library costs nothing.
  #autoWarmTimer: ReturnType<typeof setTimeout> | null = null
  private scheduleAutoWarm(): void {
    if (this.#autoWarmTimer) clearTimeout(this.#autoWarmTimer)
    const AUTO_WARM_INITIAL_MS = 60_000
    const AUTO_WARM_RECHECK_MS = 6 * 3600_000
    const tick = async (): Promise<void> => {
      try {
        await this.settings.ensureLoaded()
        if (this.settings.autoWarmCredits && this.session?.signedIn) {
          // Newly imported albums also need their genre crate. Only fills gaps
          // in an index the user has already built (a non-rebuild start skips
          // every album already indexed) — a fresh install isn't signed up
          // for a whole-library genre walk by this switch.
          if (this.genreIndex.active && !this.genreIndex.progress.running) {
            await this.genreIndex.start()
          }
          if (!this.warmerProgress.running) await this.startLibraryWarmer({ creditsOnly: true })
        }
      } catch {
        // best-effort background work; the next tick tries again
      }
      this.#autoWarmTimer = setTimeout(() => void tick(), AUTO_WARM_RECHECK_MS)
    }
    this.#autoWarmTimer = setTimeout(() => void tick(), AUTO_WARM_INITIAL_MS)
  }
  ...
  async startLibraryWarmer(opts: { creditsOnly?: boolean } = {}): Promise<void> {
    const serverId =
    ...
    await window.cratedigger.plex.startWarmer({ serverId, items, creditsOnly: opts.creditsOnly })
