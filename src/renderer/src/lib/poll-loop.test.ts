...
describe('PollLoop', () => {
  ...
  it('abandons a tick that never settles and keeps the loop alive', async () => {
    // A zombie tick (hung IPC, stalled socket past every network deadline)
    // must not wedge inFlight forever — that killed the Now Playing bar until
    // a page reload. The watchdog treats it as an error and keeps polling.
    let calls = 0
    let settled = 0
    const tick = vi.fn((): Promise<PollTickResult> => {
      calls++
      if (calls === 1) return new Promise(() => {}) // first tick never settles
      settled++
      return Promise.resolve('ok')
    })
    new PollLoop({ tick }).start()
    await vi.advanceTimersByTimeAsync(0) // zombie tick in flight
    expect(tick).toHaveBeenCalledTimes(1)

    await vi.advanceTimersByTimeAsync(15_000) // watchdog fires → 'error'
    await vi.advanceTimersByTimeAsync(2500) // first retry at normal cadence
    expect(tick).toHaveBeenCalledTimes(2)
    expect(settled).toBe(1)

    await vi.advanceTimersByTimeAsync(2500) // and the loop is back to normal
    expect(tick).toHaveBeenCalledTimes(3)
  })
