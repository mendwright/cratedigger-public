<script lang="ts">
  ...
  const themeName = $derived(getTheme(plexState.themeId).name)
...
<!-- data-chrome is a styling hook for per-theme CSS in app.css. `.bar` alone is
     used by five other components; nothing in JS reads this attribute. -->
<div class="bar" data-chrome="toolbar">
  ...
  <button
    class="theme-chip"
    onclick={() => plexState.openThemePicker()}
...
<style>
  ...
  /* Names the active theme while you're flipping through them. Same weight as
     the version chip beside it — a label, not a nav destination. */
  .theme-chip {
    -webkit-app-region: no-drag;
    padding: 0.2rem 0.55rem;
    border: none;
