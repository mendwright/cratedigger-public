...
<style>
  ...
  .panel {
    ...
    /* This panel is a fixed dark surface in every theme, but its children
       color themselves with the theme's ink tokens — which are dark inks on a
       light theme (dark-on-dark, unreadable). Re-scope the *inks* to fixed
       light ones, matching the white alphas the tints below already use.

       --brick is deliberately NOT re-scoped: it used to be pinned to a baked
       liner-notes orange, which overrode every theme's accent. Each theme's
       own accent reads fine on this surface. */
    --espresso: rgba(255, 255, 255, 0.92);
    --walnut: rgba(255, 255, 255, 0.78);
    ...
    box-shadow: 0 14px 40px rgba(var(--shadow-color), 0.22);
    z-index: 60;
