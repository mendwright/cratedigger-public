...
/**
 * Rank MB release candidates for an inbox folder: only exact track-count
 * matches qualify; among those prefer US > worldwide > other country, Official
 * status, no disambiguation, then earliest date (original beats reissue), then
 * lex mbid. Returns null when nothing has the right track count. Pure; exported
 * for testing.
 */
...
export function guessCommonTag(
  ...
  const vals = folder.files.map((f) => f.tags[key]).filter((v): v is string => !!v)
...
export class InboxController {
  ...
  async scanCompleteness(): Promise<void> {
    ...
    // pending state, then resolve in parallel — the MB pacer queues them.
    const seed: Record<string, FolderCompleteness> = { ...this.completeness }
    for (const f of todo) {
      const artist = guessCommonTag(f, 'albumartist') ?? guessCommonTag(f, 'artist') ?? ''
      ...
      await Promise.all(
        todo.map(async (f) => {
          ...
          try {
            ...
            const best = pickBestCandidate(candidates, f.file_count)
  ...
  openFolderDetail(folder: InboxFolder): void {
    this.openFolder = folder
    const guessArtist =
      guessCommonTag(folder, 'albumartist') ?? guessCommonTag(folder, 'artist') ?? ''
    const guessTitle = guessCommonTag(folder, 'album') ?? ''
    ...
    if (guessArtist || guessTitle) void this.runMatchSearch()
  }
  ...
  async runMatchSearch(): Promise<void> {
    ...
    if (folder && !this.matcher.release) {
      const best = pickBestCandidate(this.matcher.candidates, folder.file_count)
      if (best) void this.pickRelease(best.mbid)
    }
  }
  ...
  async pickRelease(mbid: string): Promise<void> {
    ...
    // Auto-apply when the preview comes back fully clean and there's no
    // existing-album conflict — matcher + AcoustID agree on every slot AND
    // file_count == total_tracks. Skips the "ready to apply" button click
    // that the user always took anyway in this state. The dup-warning path
    // still requires a human pick (new vs. merge), so we don't auto-fire
    // when `existing` is present.
    if (preview && preview.clean && !existing) {
