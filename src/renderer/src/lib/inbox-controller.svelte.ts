...
/** The pick→preview→apply state for the currently open inbox folder. */
...
export class InboxController {
  ...
  match = $state<InboxMatch | null>(null)
  ...
  async deleteFilesInOpen(paths: string[]): Promise<void> {
    ...
    try {
      ...
      if (this.match) {
        ...
        // that may no longer exist. Drop it and reconcile again against the
        // already-picked release. Dropping it without re-running strands the
        // picker on "reconciling…" forever, since nothing else re-triggers
        // once a release is picked.
        const folderPath = this.match.folderPath
        this.match = {
          ...this.match,
        ...
        if (release) {
          // Deliberately no auto-apply here, unlike pickRelease: the user
          // clicked "delete duplicates", not "import". Moving files into the
          // library off the back of a cleanup click is a surprise.
          await this.#reconcile(refreshed, release.release_mbid, folderPath)
        }
      }
    } catch (err) {
      this.error = err instanceof Error ? err.message : String(err)
  ...
  async pickRelease(mbid: string): Promise<void> {
    ...
    const preview = await this.#reconcile(folder, release.release_mbid, m.folderPath, {
  ...
  /** Run the reconciler for an already-picked release and fold the result into
   *  {@link match}: matcher + AcoustID side-by-side per file. Clean preview =
   *  both engines agree on every slot; renderer can commit immediately on
   *  click. Dirty = picker UI required.
   *
   *  Called on release pick AND whenever the folder's file set changes
   *  underfoot (a dedupe delete), because the cached preview's rows reference
   *  paths that may no longer exist. Without the second call site the picker
   *  sits on `preview === null` forever showing "reconciling…".
   *
   *  Bails without writing if the open folder changed while the request was in
   *  flight. Returns the fresh preview, or null if the service refused. */
  async #reconcile(
    folder: InboxFolder,
    releaseMbid: string,
    folderPath: string,
    extra: Partial<InboxMatch> = {}
    ...
    try {
      ...
      if (resp.ok && resp.mode === 'preview') {
        ...
        if (!preview.clean) {
          ...
          // Direction matters: more files than tracks is a duplicate-copy
          // problem with a one-click fix, not a missing-files problem. Saying
          // "incomplete" for both sends the user hunting for the wrong thing.
          if (preview.file_count > preview.total_tracks) {
            bits.push(
              `${preview.file_count} files for a ${preview.total_tracks}-track release — looks like duplicate copies`
            )
          } else if (preview.file_count < preview.total_tracks) {
            bits.push(
              `only ${preview.file_count} of ${preview.total_tracks} tracks in the release are here`
            )
          }
          matchHint =
            bits.length > 0
    ...
    if (!this.match || this.match.folderPath !== folderPath) return preview
    this.match = {
      ...this.match,
      preview,
      decisions,
