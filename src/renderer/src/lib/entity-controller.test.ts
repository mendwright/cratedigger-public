...
describe('resolving state (page shown before the MBID is known)', () => {
  it('paints the name immediately and hands off cleanly to load()', () => {
    const { c } = makeController()
    c.beginResolving('Sluice')
    expect(c.resolving).toBe(true)
    expect(c.entity?.name).toBe('Sluice')
    expect(c.loading).toBe(true)
    expect(c.mbid).toBeNull()
    c.load('mbid-sluice', 'Sluice')
    expect(c.resolving).toBe(false)
    expect(c.mbid).toBe('mbid-sluice')
  })

  it('cancelResolving only tears down a page that never resolved', () => {
    const { c } = makeController()
    c.beginResolving('Nobody')
    c.cancelResolving()
    expect(c.entity).toBeNull()
    c.load('mbid-x', 'Someone')
    c.cancelResolving()
    expect(c.entity?.name).toBe('Someone')
  })
})
