...
describe('unanimousReleaseMbid', () => {
  const REL = '21baab2d-4562-4bfa-a958-5942a6aa5937'
  ...
  it('rejects non-UUID junk written into the MusicBrainz field by foreign taggers', () => {
    const f = folder({ tags: [{}, {}] })
    f.files[0].tags.musicbrainz_albumid = '1064174'
    f.files[1].tags.musicbrainz_albumid = '1064174'
    expect(unanimousReleaseMbid(f)).toBeNull()
  })
})
...
describe('InboxController — folder detail', () => {
  ...
  it('opens a unanimously embedded MusicBrainz release directly without text search', async () => {
    ...
    detailed.files[0].tags.musicbrainz_albumid = '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0001'
    const { c, tagger, plex } = makeController({
      tagger: {
        getFolder: vi.fn(async () => detailed),
        getRelease: vi.fn(async () => ({
          release_mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0001',
    ...
    expect(tagger.getRelease).toHaveBeenCalledWith({ mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0001' })
  ...
  it('rejects an embedded box-set release when its track count exceeds the folder', async () => {
    ...
    detailed.files[0].tags.musicbrainz_albumid = '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0002'
    const { c, tagger, plex } = makeController({
      tagger: {
        getFolder: vi.fn(async () => detailed),
        getRelease: vi.fn(async () => ({
          release_mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0002',
    ...
    expect(tagger.getRelease).toHaveBeenCalledWith({ mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0002' })
  ...
  it('keeps a near-complete embedded release instead of choosing another performance', async () => {
    ...
    detailed.files.forEach((file, i) => {
      file.tags.musicbrainz_albumid = '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0003'
    ...
    const { c, tagger, plex } = makeController({
      tagger: {
        getFolder: vi.fn(async () => detailed),
        getRelease: vi.fn(async () => ({
          release_mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0003',
    ...
    expect(tagger.getRelease).toHaveBeenCalledWith({ mbid: '5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0003' })
    expect(c.matcher.release?.release_mbid).toBe('5f1e0f14-dfe3-4a2f-9d75-6a0f3f0a0003')
