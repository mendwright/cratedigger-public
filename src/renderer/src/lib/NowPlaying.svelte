    ...
    <div class="center">
      <div class="controls">
        ...
        <button
          ...
          title="Lyrics — theater mode (⌘T)"
          aria-label="Lyrics — theater mode"
        >
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
...
<style>
  .bar {
    ...
    /* Deliberately not draggable. This bar used to be `drag` with each
       transport control punching a `no-drag` hole in it — the same
       arrangement that killed the top toolbar's buttons (see TopToolbar's
       .drag-fill comment). macOS caches those hole rects, and this bar is
       about the worst case for staleness: it's pinned to the bottom edge and
       laid out in minmax() fr columns, so its geometry moves on every resize
       in both axes. A stale set of holes means the strip behaves as one drag
       region and play/pause/next stop responding until a reload.

       Window dragging lives on the toolbar's fixed handle, so there's nothing
       to gain here worth that risk. No drag region means no rect to go
       stale. Don't make this bar draggable again. */
  ...
  .left img, .ph {
    ...
    flex-shrink: 0;
    box-shadow: 0 4px 14px rgba(var(--shadow-color), 0.22), 0 0 0 1px var(--ink-tint-04);
  }
  .meta { min-width: 0; }
  .title {
    font-size: 0.9rem;
  ...
  .bar-thumb {
    ...
    border-radius: var(--radius-pill);
    background: var(--espresso);
    box-shadow: 0 0 0 1px rgba(var(--shadow-color), 0.18), 0 2px 6px rgba(var(--shadow-color), 0.22);
    opacity: 0;
  ...
  .queue-btn.active { color: var(--brick); background: color-mix(in srgb, var(--brick) 15%, transparent); }
