<script lang="ts">
  ...
  const commands: Command[] = [
    ...
    { id: 'go-charts',   label: 'Go to charts',          category: 'Navigation', keys: [mod, '8'], run: () => void plexState.openCharts() },
    { id: 'go-playlists', label: 'Go to playlists',      category: 'Navigation', run: () => void plexState.openPlaylists() },
...
<style>
  .backdrop {
    position: fixed;
    inset: 0;
    background: rgba(var(--shadow-color), 0.42);
