<script lang="ts">
  ...
  const crates = $derived.by(() => {
    ...
    for (const a of source) {
      // MusicBrainz genres when the index has them, Plex's otherwise. Plex's
      // raw tags are too coarse and too inconsistent to make crates out of —
      // one library had 551 albums under "Rock" and 355 genres used once.
