<script lang="ts">
  ...
  $effect(() => {
    if (
      ...
      draftKioskUrl = plexState.settings.kioskUrl
      ...
      draftAutoWarm = plexState.settings.autoWarmCredits
      synced = true
    }
  ...
  >
    <div class="modal" role="dialog" aria-modal="true" aria-label="Settings" tabindex="-1">
      ...
      <div class="body">
        ...
        <label class="field">
          <div class="field-label">Kiosk URL (theme mirroring)</div>
          <div class="field-hint">
            When set, the now-playing kiosk on that host wears whatever theme
            you pick here — the palette is sent over on every switch. Leave
            blank to disable.
          </div>
          <input
            type="text"
            bind:value={draftKioskUrl}
            placeholder="http://kiosk-host:8843"
            spellcheck="false"
            autocomplete="off"
          />
        </label>
        {/if}
        <label class="field toggle-field">
          <input type="checkbox" bind:checked={draftAutoWarm} />
          <div class="toggle-text">
            <div class="field-label">Keep every album's credits on this Mac</div>
            <div class="field-hint">
              A minute after launch, quietly pulls MusicBrainz credits for any
              album that isn't cached yet and keeps going until the whole library
              is on disk — then checks for new albums every few hours. Paced at
              about one album every three seconds, so a big library takes a few
              evenings with the app open; it picks up where it left off.
            </div>
          </div>
        </label>
