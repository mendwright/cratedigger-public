...
<!-- data-view is a styling hook for the per-theme CSS in app.css. Both album
     detail and Now Playing render this shell, so one attribute themes both;
     `header`, `.layout`, `.art` and `.body` are far too generic to target. -->
<section data-view="album">
  <header>
    {@render header()}
  </header>
  ...
  {:else if phase === 'ready' && detail}
    <div class="layout">
      <aside class="art" data-album-art oncontextmenu={onArtContextMenu} role="presentation">
        {#if src}
          <img
            src={src}
...
<style>
  ...
  .cover-fallback {
    /* Sized so the cover AND the facts block under it fit without scrolling —
       ...
       Exposed as a custom property because per-theme CSS needs to lay an
       overlay exactly over the cover (see Zine's halftone in app.css), and
       .art is taller than the cover — it also holds the action row and the
       facts block. Two copies of this clamp would drift apart silently. */
    max-width: var(--album-cover-max);
  }
  .art {
    --album-cover-max: clamp(240px, calc(100vh - 25rem), 720px);
  }
  .art img {
    width: 100%;
    aspect-ratio: 1/1;
    ...
    box-shadow:
      0 28px 70px rgba(var(--shadow-color), 0.3),
