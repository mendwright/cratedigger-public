<script lang="ts">
  ...
  import { classicalTrackPerformers, modelAssertsRepertoire } from '../../../../shared/classical-model'
...
{#snippet trackRow(t: PlexTrack, label?: string)}
    {@const mbArtists = classical
      ? classicalTrackPerformers(credits?.byTrack?.[t.ratingKey] ?? [])
      : credits?.trackArtists?.[t.ratingKey] ?? []}
