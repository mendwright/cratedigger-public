<script lang="ts">
  ...
  import { editionChips, tokenizeSummary } from './helpers'
...
<h1 data-album-title>{detail.album.title}</h1>
...
<div class="album-actions" data-album-actions>
...
<style>
  ...
  .g-sep {
    color: inherit;
    /* The separator's trailing space is the end of its inline box, and a flex
       parent strips it — the line read "Hard Rock,Rock,Classic Rock". */
    white-space: pre;
  }
  .edition-row {
    display: flex;
  ...
  .play-album {
    ...
    cursor: pointer;
    box-shadow:
      0 8px 22px color-mix(in srgb, var(--brick) 28.0%, transparent),
      inset 0 1px 0 rgba(var(--shadow-color), 0.18);
    transition: background 140ms ease, transform 120ms ease, box-shadow 140ms ease;
  }
  .play-album:not(:disabled):hover {
    transform: translateY(-1px);
    box-shadow:
      0 12px 28px color-mix(in srgb, var(--brick) 40%, transparent),
