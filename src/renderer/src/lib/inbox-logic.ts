...
export function performanceConflict(
  ...
  // MusicBrainz separates how an artist was *credited* from who they *are*:
  // The Angels' US pressings are credited "Angel City" on the same artist
  // entity. Files tagged with the artist's real name are the right
  // performance, so a match on the canonical names clears the guard.
  const canonicalNames = (release.albumartist_names ?? [])
    .map((n) => normalizeCredit(n))
    .filter((n) => n.length > 0)
  const matchesCanonical = (credit: string): boolean =>
    canonicalNames.some((name) => sameCreditedName(credit, name))
  ...
  // A conductor or orchestra tag is only evidence of *whose performance this
  // is* when the release is a performance credit. Well-tagged pop records
  // stamp the whole session — King Gizzard's "Phantom Island" carries
  // `conductor: Brett Kelly` for the orchestra they hired. When the files'
  // unanimous albumartist IS the entire release credit, the release is
  // credited solely to that artist and those tags are sidemen, not identity.
  // Requiring whole-string equality keeps the guard strict for classical,
  // where the release credit reads "Beethoven; Karajan; …" and so never
  // equals a lone albumartist.
  const albumartist = unanimous('albumartist')
  if (albumartist && releaseCredit && normalizeCredit(albumartist) === releaseCredit) {
    return null
  }
  if (albumartist && matchesCanonical(albumartist)) return null
  ...
  const performer = albumartist ?? unanimous('artist')
  ...
  if (performerEntities.some((credit) => matchesReleaseEntity(credit, releaseEntities)))
    return null
  if (matchesCanonical(performer) || performerEntities.some(matchesCanonical)) return null
