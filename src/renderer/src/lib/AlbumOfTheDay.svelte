...
<style>
  .card {
    ...
    background-image:
      radial-gradient(var(--ink-tint-04) 1px, transparent 1px),
      linear-gradient(180deg, var(--surface), var(--paper));
    background-size: 4px 4px, auto;
    border: 1px solid var(--hairline);
    border-radius: var(--radius-md);
    box-shadow: 0 18px 40px rgba(var(--shadow-color), 0.18), inset 0 0 0 1px rgba(var(--shadow-color), 0.4);
  }
  ...
  .cover {
    ...
    overflow: hidden;
    cursor: pointer;
    box-shadow: 0 14px 32px rgba(var(--shadow-color), 0.32);
    transition: transform 240ms ease, box-shadow 240ms ease;
  }
  .cover:hover {
    transform: translateY(-3px);
    box-shadow: 0 22px 44px rgba(var(--shadow-color), 0.42);
  }
  ...
  .play:disabled { background: color-mix(in srgb, var(--brick) 40%, transparent); color: var(--inset); cursor: not-allowed; }
  .open {
    background: transparent;
    color: var(--walnut);
    border: 1px solid var(--hairline);
