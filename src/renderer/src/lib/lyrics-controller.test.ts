...
function makeController(
  ...
  const deps: LyricsDeps = {
    ...
    nowPlayingTrackRatingKey: opts.nowPlaying,
...
describe('LyricsController — prefetchAlbum', () => {
  ...
  it('stops pulling new tracks once the open album changes mid-flight', async () => {
    // The worker pool re-checks the open album before grabbing each track, so a
    // mid-flight navigation halts the queue. Let the first wave of workers
    // proceed (guard returns 'album-1'), then flip to 'other' so the remaining
    // tracks are never fetched.
    let calls = 0
    const currentAlbumRatingKey = vi.fn(() => (++calls <= 4 ? 'album-1' : 'other'))
    const plex = makePlex()
    ...
    await c.prefetchAlbum(detail('album-1', keys))
    expect(plex.getLyrics.mock.calls.length).toBeLessThan(keys.length)
    expect(c.byTrack.h).toBeUndefined()
  })

  it('pulls the currently playing track to the front of the queue', async () => {
    const { c, plex } = makeController({ nowPlaying: () => 'g' })
    await c.prefetchAlbum(detail('album-1', ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']))
    expect(plex.getLyrics.mock.calls[0][0].title).toBe('T-g')
    expect(plex.getLyrics).toHaveBeenCalledTimes(8) // everything still fetched
  })

  it('does not reorder when the playing track is already cached or off-album', async () => {
    const cached = makeController({ nowPlaying: () => 'c' })
    cached.c.byTrack = { c: lyricsResult }
    await cached.c.prefetchAlbum(detail('album-1', ['a', 'b', 'c']))
    expect(cached.plex.getLyrics.mock.calls[0][0].title).toBe('T-a')

    const offAlbum = makeController({ nowPlaying: () => 'not-here' })
    await offAlbum.c.prefetchAlbum(detail('album-1', ['a', 'b']))
    expect(offAlbum.plex.getLyrics.mock.calls[0][0].title).toBe('T-a')
    expect(offAlbum.plex.getLyrics).toHaveBeenCalledTimes(2)
  })
})
