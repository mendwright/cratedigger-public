...
describe('performanceConflict — credit name vs canonical artist name', () => {
  function bandFolder(albumartist: string): InboxFolder {
    const file = audio('01 - Take a Long Line.flac', '1')
    file.tags.albumartist = albumartist
    file.tags.artist = albumartist
    return { folder: 'Face to Face', path: '/inbox/Face to Face', file_count: 1, files: [file] }
  }

  it('accepts files tagged with the artist behind a differently-credited release', () => {
    // The Angels' US pressings are credited "Angel City" on the same MB artist.
    const rel = release(1)
    rel.albumartist = 'Angel City'
    rel.albumartist_names = ['The Angels']
    expect(performanceConflict(bandFolder('The Angels'), rel)).toBeNull()
  })

  it('still blocks a genuinely different band', () => {
    const rel = release(1)
    rel.albumartist = 'Angel City'
    rel.albumartist_names = ['The Angels']
    expect(performanceConflict(bandFolder('Angel Olsen'), rel)).toMatch(/Performance mismatch/)
  })

  it('keeps the old behaviour when the service sends no canonical names', () => {
    const rel = release(1)
    rel.albumartist = 'Angel City'
    expect(performanceConflict(bandFolder('The Angels'), rel)).toMatch(/Performance mismatch/)
  })
})
