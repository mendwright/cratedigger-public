...
export function albumMatchesFilters(
  ...
  // The album's effective genres — MusicBrainz's when the genre index has
  // them, Plex's otherwise. Defaults to Plex's so callers that predate the
  // genre index keep working. Passed in rather than read off `album.genres`
  // because the MB genres live in a separate map keyed by ratingKey, and a
  // crate the user picked off the rail may not exist in Plex at all.
  genresForAlbum: string[] = album.genres
): boolean {
  if (filters.genreInclude.size > 0) {
    let hit = false
    for (const g of genresForAlbum) {
