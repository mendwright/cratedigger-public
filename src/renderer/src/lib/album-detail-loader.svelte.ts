...
export class AlbumDetailLoader {
  ...
  async loadLabelRail(): Promise<void> {
    ...
    for (const n of [...indexed, ...studio]) {
      ...
      // labels ("[no label]", "[unknown]") — not a shared label, so "more from
      // [no label]" would just be a shelf of unrelated self-released albums.
      if (c && !(c.startsWith('[') && c.endsWith(']'))) names.add(c)
    }
  ...
  private async resolveAlbumArtistMbid(
    ...
    for (const mbid of candidates) {
      try {
        ...
        const release = await this.tagger.getRelease({ mbid, timeoutMs: 5000 })
