...
import type { LabelReleaseGroup, MbArtistReleaseGroup } from '../../../shared/plex'
import {
  roleRank,
  ROLE_RANK,
  groupAppearances,
...
function rg(opts: {
  ...
  return {
    mbid: opts.mbid,
    title: opts.title,
    firstReleaseDate: opts.date ?? null,
    primaryType: opts.primaryType ?? 'Album',
    secondaryTypes: opts.secondaryTypes ?? [],
    disambiguation: null
  }
}
...
describe('missingFromLibrary', () => {
  it('returns MB albums/EPs the library lacks, chronological', () => {
    const groups = [
      rg({ mbid: 'g1', title: 'Radial Gate', date: '2023-04-01' }),
      rg({ mbid: 'g2', title: 'Companion', date: '2026-01-01' }),
      rg({ mbid: 'g3', title: 'Fifteen', date: '2019-06-01' })
    ]
    const lib = [libAlbum({ ratingKey: 'a1', title: 'Companion', year: 2026 })]
    const out = missingFromLibrary(groups, ['Sluice'], lib)
    expect(out.map((m) => m.title)).toEqual(['Fifteen', 'Radial Gate'])
  })

  it('matches by MBID even when titles diverge', () => {
    const uuid = 'a1b2c3d4-e5f6-4789-8abc-def012345678'
    const groups = [rg({ mbid: uuid, title: 'Radial Gate (Deluxe Whatever)' })]
    const lib = [
      libAlbum({
        ratingKey: 'a1',
        title: 'Totally Renamed',
        guids: [`mbid://release-group/${uuid}`]
      })
    ]
    expect(missingFromLibrary(groups, ['Sluice'], lib)).toEqual([])
  })

  it('accepts any of the candidate artist names', () => {
    const groups = [rg({ mbid: 'g1', title: 'Winter Journal' })]
    const lib = [libAlbum({ ratingKey: 'a1', title: 'Winter Journal', artist: 'J. Morris' })]
    expect(missingFromLibrary(groups, ['Sluice'], lib)).toHaveLength(1)
    expect(missingFromLibrary(groups, ['Sluice', 'J. Morris'], lib)).toEqual([])
  })

  it('drops singles, live/compilation shapes, and duplicate titles', () => {
    const groups = [
      rg({ mbid: 'g1', title: 'Hit Song', primaryType: 'Single' }),
      rg({ mbid: 'g2', title: 'Live at the Cradle', secondaryTypes: ['Live'] }),
      rg({ mbid: 'g3', title: 'Best Of', secondaryTypes: ['Compilation'] }),
      rg({ mbid: 'g4', title: 'Real Album', date: '2020' }),
      rg({ mbid: 'g5', title: 'Real Album', date: '2020-02-01' }),
      rg({ mbid: 'g6', title: 'Little One', primaryType: 'EP', date: '2021' })
    ]
    const out = missingFromLibrary(groups, ['Sluice'], [])
    expect(out.map((m) => m.mbid)).toEqual(['g4', 'g6'])
    expect(out[1].primaryType).toBe('EP')
  })
})

function labelRg(opts: {
  rgMbid: string
  title: string
  artist: string
  date?: string | null
...
describe('labelMissingFromLibrary', () => {
  it('diffs the label catalog per release-group artist, chronological', () => {
    const groups = [
      labelRg({ rgMbid: 'g1', title: 'Young Team', artist: 'Mogwai', date: '1997-10-21' }),
      labelRg({ rgMbid: 'g2', title: 'Philophobia', artist: 'Arab Strap', date: '1998-04-06' }),
      labelRg({ rgMbid: 'g3', title: 'Peloton', artist: 'The Delgados', date: '1998-06-01' })
    ]
    const lib = [libAlbum({ ratingKey: 'a1', title: 'Young Team', artist: 'Mogwai' })]
    const out = labelMissingFromLibrary(groups, lib)
    // Both 1998 → title tiebreak puts Peloton first.
    expect(out.map((m) => m.title)).toEqual(['Peloton', 'Philophobia'])
    expect(out[1].artist).toBe('Arab Strap')
  })

  it('a same-titled album by a different artist does not count as owned', () => {
    const groups = [labelRg({ rgMbid: 'g1', title: 'Dusk', artist: 'The Sea and Cake' })]
    const lib = [libAlbum({ ratingKey: 'a1', title: 'Dusk', artist: 'Badly Drawn Boy' })]
    expect(labelMissingFromLibrary(groups, lib)).toHaveLength(1)
  })

  it('drops singles and reissue shapes, and dedupes artist+title', () => {
    const groups = [
      labelRg({ rgMbid: 'g1', title: 'Hit', artist: 'Mogwai', primaryType: 'Single' }),
      labelRg({ rgMbid: 'g2', title: 'Best Of', artist: 'Mogwai', secondaryTypes: ['Compilation'] }),
      labelRg({ rgMbid: 'g3', title: 'Young Team', artist: 'Mogwai', date: '1997' }),
      labelRg({ rgMbid: 'g4', title: 'Young Team', artist: 'Mogwai', date: '1997-10-21' })
    ]
    expect(labelMissingFromLibrary(groups, []).map((m) => m.mbid)).toEqual(['g3'])
  })
})
