<script lang="ts">
  ...
  import { formatListened } from '../../../shared/charts'
  ...
  function open(e: ChartEntry): void {
    if (c.dimension === 'artists') void plexState.openArtistByName(e.title, { plexRatingKey: e.ratingKey })
...
<div class="page">
  ...
  {#if c.users.length > 0}
    ...
    <!-- One click = that person's chart. ⌘/⇧-click adds or removes people to
         compare. `everyone` is its own pill; while it's on, no individual name
         lights up, so the row reads as a single choice rather than a pile of
         toggles you have to switch off one by one. -->
    <div class="people">
      <button class="all" class:on={c.allSelected} onclick={() => c.selectAll()} title="Everyone's listening, combined">everyone</button>
      {#each c.users as u (u.userId)}
        <button
          class="chip"
          class:on={!c.allSelected && c.selected.has(u.userId)}
          onclick={(e) => (e.metaKey || e.shiftKey || e.ctrlKey ? c.toggleUser(u.userId) : c.onlyUser(u.userId))}
          title="{u.trackPlays.toLocaleString()} plays — click for just {u.name}, ⌘-click to compare"
        >{u.name}</button>
      {/each}
    </div>
        ...
        {#each c.result.entries as e (e.rank + e.title)}
          ...
          <button class="card" onclick={() => open(e)} title={e.subtitle ? `${e.title} — ${e.subtitle}` : e.title}>
            ...
            <div class="meta">
              ...
              {#if e.seconds > 0}
                <span class="h">{formatListened(e.seconds)}</span>
                <span class="p">{e.plays.toLocaleString()} {e.plays === 1 ? 'play' : 'plays'}</span>
              {:else}
                <span class="h">{e.plays.toLocaleString()} {e.plays === 1 ? 'play' : 'plays'}</span>
              {/if}
            </div>
          </button>
        {/each}
...
<style>
  ...
  .card:hover .art .fallback {
    box-shadow: 0 5px 14px rgba(var(--shadow-color), 0.3);
  }
  .card:hover .t {
    color: var(--brick);
  }
  ...
  .art img {
    ...
    background: var(--surface);
    box-shadow: 0 1px 3px rgba(var(--shadow-color), 0.18);
  ...
  .badge {
    ...
    box-shadow: 0 1px 2px rgba(var(--shadow-color), 0.25);
  }
  .meta {
    display: flex;
    flex-direction: column;
