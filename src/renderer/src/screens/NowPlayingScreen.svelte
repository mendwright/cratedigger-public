...
<style>
  ...
  .up-next {
    ...
    /* Fixed dark surface in every theme — re-scope the ink tokens the same
       way QueuePanel does, so the title isn't dark-on-dark on light themes. */
    --espresso: #f2ead9;
    ...
    backdrop-filter: blur(14px);
    -webkit-backdrop-filter: blur(14px);
    box-shadow: 0 12px 32px rgba(43, 29, 18, 0.22);
    cursor: pointer;
    text-align: left;
    max-width: 260px;
    color: inherit;
  ...
  .up-next:hover {
    border-color: rgba(140, 53, 34, 0.4);
  ...
  .up-next-ph {
    ...
    object-fit: cover;
    flex-shrink: 0;
    box-shadow: 0 4px 12px rgba(43, 29, 18, 0.18);
  }
  .up-next-ph {
