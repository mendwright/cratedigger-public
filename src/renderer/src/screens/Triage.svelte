...
<style>
  ...
  header {
    ...
    align-items: center;
    gap: 1rem;
    /* Not draggable — full-width header with `no-drag` holes, the arrangement
       that goes stale on macOS. See TopToolbar's .drag-fill. */
  ...
  .dup-items li.best {
    background: color-mix(in srgb, var(--ok-fg) 5%, transparent);
    box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--ok-fg) 18%, transparent);
    padding-left: 0.5rem;
    padding-right: 0.5rem;
  }
  .best-badge {
    ...
    border: 1px solid color-mix(in srgb, var(--ok-fg) 30%, transparent);
