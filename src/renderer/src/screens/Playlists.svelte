<script lang="ts">
  ...
  // "42 tracks · 3 hr 11 min" — the right pane showed only a name, so there
  // was no way to tell a 12-track playlist from a 400-track one without
  // counting. leafCount/durationMs come back with the list itself, so this
  // costs nothing extra.
  const openSub = $derived.by(() => {
    if (!open) return ''
    const parts: string[] = []
    parts.push(`${open.leafCount.toLocaleString()} ${open.leafCount === 1 ? 'track' : 'tracks'}`)
    const dur = open.durationMs ? fmtLongDuration(open.durationMs) : ''
    if (dur) parts.push(dur)
    return parts.join(' · ')
  })

  // fmtMs is mm:ss, which reads as nonsense past an hour of playlist.
  ...
  let detailMode = $state<'tracks' | 'albums'>('tracks')
  ...
  const visibleAlbums = $derived.by(() => {
    const albumsByKey = new Map(
      (plexState.allAlbums ?? []).map((album) => [album.ratingKey, album])
    )
    const seen = new Set<string>()
    const albums: PlexAlbum[] = []
    for (const row of visibleRows) {
      if (row.kind !== 'matched') continue
      const ratingKey = row.item.albumRatingKey
      if (!ratingKey || seen.has(ratingKey)) continue
      const album = albumsByKey.get(ratingKey)
      if (!album) continue
      seen.add(ratingKey)
      albums.push(album)
    }
  ...
  $effect(() => {
    if (detailMode === 'albums' && open) void plexState.ensureAllAlbumsLoaded()
  })
...
<section class="pl" data-view="playlists">
  <header>
  ...
  <div class="body">
    ...
    <div class="detail-col">
      ...
      {:else}
        <div class="d-head">
          <div class="d-ident">
            <div class="d-kicker">playlist</div>
          {#if renaming}
          ...
          {/if}
            {#if openSub}<div class="d-sub">{openSub}</div>{/if}
          </div>
          <div class="d-actions">
            <div class="view-toggle" aria-label="Playlist view">
              <button
                class:active={detailMode === 'tracks'}
                aria-pressed={detailMode === 'tracks'}
                onclick={() => (detailMode = 'tracks')}
                title="Track list"
              >
                tracks
              </button>
              <button
                class:active={detailMode === 'albums'}
                aria-pressed={detailMode === 'albums'}
                onclick={() => (detailMode = 'albums')}
                title="Album art"
              >
                art
              </button>
        ...
        {:else}
          {#if detailMode === 'albums'}
            {#if plexState.allAlbumsLoading && visibleAlbums.length === 0}
              <p class="hint">loading album art…</p>
            {:else if visibleAlbums.length === 0}
              <p class="hint">no library albums found for these tracks.</p>
            {:else}
              <ul class="album-grid" data-grid="albums">
                {#each visibleAlbums as album (album.ratingKey)}
                  <li><AlbumCard {album} /></li>
                {/each}
              </ul>
            {/if}
          {:else}
...
<style>
  .d-ident { display: flex; flex-direction: column; align-items: flex-start; gap: 0.1rem; min-width: 0; }
  .d-kicker {
    font-size: 0.7rem;
  ...
  .d-sub { font-size: 0.78rem; color: var(--faded); }
  .pl {
    display: flex;
  ...
  .chip:hover { background: color-mix(in srgb, var(--brick) 22%, transparent); }
  ...
  .m-badge.ok { color: var(--brick); background: color-mix(in srgb, var(--brick) 12%, transparent); }
  ...
  .view-toggle button:hover { color: var(--espresso); }
