...
<div class="page">
  <header class="head">
    <div>
      <button class="back" onclick={() => plexState.closeListenLater()} aria-label="Back to library">←</button>
      <h1>Listen Later</h1>
      {#if entries.length > 0}
        <div class="sub">{entries.length} album{entries.length === 1 ? '' : 's'} saved</div>
      {/if}
    </div>
  </header>
...
<style>
  ...
  .cover {
    ...
    box-shadow: 0 6px 20px rgba(43, 29, 18, 0.18);
    transition: transform 140ms ease, box-shadow 140ms ease;
  }
  .card:hover .cover {
    transform: translateY(-2px);
    box-shadow: 0 12px 28px rgba(43, 29, 18, 0.28);
  }
  .cover img {
    width: 100%;
    height: 100%;
