<script lang="ts">
  ...
  import { parseMusicBrainzReleaseInput } from '../../../shared/guid'
  ...
  let releaseIdInput = $state('')
  let releaseIdError = $state<string | null>(null)
  let releaseIdFolderPath = $state<string | null>(null)

  $effect(() => {
    const folderPath = openFolder?.path ?? null
    if (folderPath !== releaseIdFolderPath) {
      releaseIdFolderPath = folderPath
      releaseIdInput = ''
      releaseIdError = null
    }
  })

  function onReleaseIdSubmit(e: SubmitEvent): void {
    e.preventDefault()
    const mbid = parseMusicBrainzReleaseInput(releaseIdInput)
    if (!mbid) {
      releaseIdError = 'Paste a MusicBrainz release ID or release URL.'
      return
    }
    releaseIdError = null
    void plexState.inbox.pickRelease(mbid)
  }

  // --- Keyboard flow -------------------------------------------------------
  // Folder list: j/k select, Enter opens. Folder detail: 1–9 pick an MB
  // candidate, Cmd+Enter applies, Esc backs out to the list. Plain letters
  // stand down while typing (the match-search inputs) or when an overlay
  // owns the keyboard.
  let selPath = $state<string | null>(null)
  const selIndex = $derived(selPath === null ? -1 : folders.findIndex((f) => f.path === selPath))
    ...
    const next =
      selIndex < 0
        ? delta > 0
          ? 0
          : folders.length - 1
        : Math.max(0, Math.min(folders.length - 1, selIndex + delta))
    selPath = folders[next].path
    queueMicrotask(() => {
      if (selPath === null) return
      document
        .querySelector(`[data-inbox-row="${CSS.escape(selPath)}"]`)
        ?.scrollIntoView({ block: 'nearest' })
    })
  }

  function onInboxKey(e: KeyboardEvent): void {
    if (e.altKey) return
    const cmd = e.metaKey || e.ctrlKey
    if (overlayOwnsKeys()) return

    // Cmd+Enter applies even from inside a text input — no conflict there.
    if (cmd && e.key === 'Enter') {
      if (openFolder && matcher.release && readyToApply && !match?.applying) {
        e.preventDefault()
        void plexState.inbox.applyRelease()
      }
      return
    }
    if (cmd || isTextTarget(e)) return
    ...
    if (openFolder === null) {
      switch (e.key) {
        ...
        case 'o':
          if (selIndex >= 0) {
            e.preventDefault()
            void plexState.inbox.openFolderDetail(folders[selIndex])
          }
          return
      }
      return
    ...
    // Folder detail
    if (e.key === 'Escape') {
      plexState.inbox.closeFolderDetail()
      return
    }
    // 1–9: pick the Nth MB candidate while the candidate list is up.
    if (/^[1-9]$/.test(e.key) && matcher.candidates.length > 0 && !matcher.release && !matcher.releaseLoading) {
      const c = matcher.candidates[Number(e.key) - 1]
      if (c) {
        e.preventDefault()
        void plexState.inbox.pickRelease(c.mbid)
      }
    }
  }
  ...
  const duplicateAssignments = $derived(duplicateTrackNumbers(preview, decisions))
  const missingReleaseTracks = $derived.by(() => {
    if (!preview || !matcher.release) return []
    const assigned = new Set<number>()
    for (const row of preview.rows) {
      const decision = decisionFor(row)
      if (typeof decision === 'number') assigned.add(decision)
    }
    return matcher.release.tracks
      .map((track, index) => ({ track, index: index + 1 }))
      .filter(({ index }) => !assigned.has(index))
  })
  const identityConflict = $derived(performanceConflict(openFolder, matcher.release))
  const readyToApply = $derived(
    isReadyToApply(preview, decisions) && identityConflict === null
  )
  ...
  const lowerQualityFiles = $derived(lowerQualityCopies(openFolder?.files ?? []))
  const missingTracks = $derived(missingTrackNumbers(openFolder?.files ?? []))

  function confirmDeleteLowerQuality(): void {
    if (lowerQualityFiles.length === 0) return
    const ok = confirm(
      `delete ${lowerQualityFiles.length} lower-quality cop${lowerQualityFiles.length === 1 ? 'y' : 'ies'}? the highest-fidelity copy of each track stays.`
    )
    if (!ok) return
    void plexState.inbox.deleteFilesInOpen(lowerQualityFiles.map((f) => f.path))
  }
  ...
  function matchStatus(): {
    ...
    if (identityConflict) {
      return {
        tone: 'warn',
        line1: 'wrong release',
        line2: 'artist credits do not match these files'
      }
    }
    if (
      preview.clean &&
    ...
    if (duplicateAssignments.length > 0) {
      parts.push(`track ${duplicateAssignments.join(', ')} assigned more than once`)
    }
    // Direction matters. More files than tracks means duplicate copies (one
    // click to fix); fewer means the download really is short. Calling both
    // "incomplete download" sends the user hunting for missing files that are
    // not missing.
    const extraFiles = preview.file_count > preview.total_tracks
    const missingFiles = preview.file_count < preview.total_tracks
    if (extraFiles) {
      parts.push(`${preview.file_count} files for ${preview.total_tracks} tracks`)
    } else if (missingFiles) {
      parts.push(`only ${preview.file_count} of ${preview.total_tracks} tracks`)
    }
    return {
      tone: 'warn',
      line1: extraFiles
        ? duplicateFiles.length > 0
          ? 'duplicate copies'
          : 'more files than tracks'
        : missingFiles
          ? 'incomplete download'
          : unresolved > 0 || duplicateAssignments.length > 0
            ? 'needs attention'
            : 'review before apply',
...
<svelte:window onkeydown={onInboxKey} />

<section class="inbox">
  <header>
    <BackButton onclick={() => plexState.closeInbox()} />
    <div class="head">
  ...
  {:else if openFolder === null}
    {@const eligibleSingles = folders.filter((f) => plexState.inbox.singleCandidate(f) !== null).length}
    {#if eligibleSingles > 1}
      {@const sb = plexState.inbox.singleBatch}
      <div class="singles-sweep">
        <button
          class="secondary"
          disabled={sb?.running}
          onclick={() => void plexState.inbox.applyAllSingles()}
          title="Every 1-file folder with artist+title tags gets filed into the Singles library"
        >
          {sb?.running ? `filing ${sb.done}/${sb.total}…` : `file all ${eligibleSingles} singles`}
        </button>
      </div>
    {/if}
    <ul class="folders">
      {#each folders as f (f.path)}
        ...
        {@const singleFile = plexState.inbox.singleCandidate(f)}
        <li class:kbd-selected={selPath === f.path} data-inbox-row={f.path}>
          <button class="folder" onclick={() => void plexState.inbox.openFolderDetail(f)}>
            <div class="f-title">
              {folderLabel(f)}
              {#if c}
          ...
          {#if singleFile}
            <button
              class="f-single"
              disabled={plexState.inbox.applyingSinglePath !== null || plexState.inbox.singleBatch?.running}
              onclick={() => void plexState.inbox.applySingleFromList(f)}
              title={`File “${singleFile.tags.title}” into the Singles library`}
            >
              {plexState.inbox.applyingSinglePath === singleFile.path ? '…' : 'single'}
            </button>
          {/if}
          <button
            class="f-trash"
    ...
    <div class="detail">
      <div class="d-head">
        ...
        <div class="d-head-title">
          ...
          <div class="sub">
            ...
            {#if missingTracks.length > 0}
              <span class="fp-pill fp-error" title="Track numbers inferred from the files">
                ! missing track{missingTracks.length === 1 ? '' : 's'} {missingTracks.join(', ')}
              </span>
            {/if}
            {#if fingerprintPill}
              <span
        ...
        <div class="files-actions">
          {#if lowerQualityFiles.length > 0}
            <button class="secondary files-dedupe" onclick={confirmDeleteLowerQuality}>
              delete {lowerQualityFiles.length} lower-quality cop{lowerQualityFiles.length === 1 ? 'y' : 'ies'}
              (keep best)
            </button>
          {/if}
      ...
      <section class="files">
        ...
        <ul>
          {#each openFolder.files as file (file.path)}
            {@const singleReady = !!(file.tags.artist || file.tags.albumartist) && !!file.tags.title}
            <li>
              <button
                class="file-del"
              ...
              <button
                class="file-single"
                disabled={!singleReady || plexState.inbox.applyingSinglePath !== null}
                onclick={() => void plexState.inbox.applySingleInOpen(file)}
                title={singleReady
                  ? 'File just this track into the Singles library (album stays un-imported)'
                  : 'Needs artist + title tags — match the folder to a release instead'}
              >
                {plexState.inbox.applyingSinglePath === file.path ? '…' : 'single'}
              </button>
      ...
      <section class="match">
        ...
        {#if match}
          ...
          <form class="release-id-search" onsubmit={onReleaseIdSubmit}>
            <label>
              <span>musicbrainz release id</span>
              <input
                type="text"
                bind:value={releaseIdInput}
                placeholder="UUID or musicbrainz.org/release URL"
                autocomplete="off"
                spellcheck="false"
                oninput={() => { releaseIdError = null }}
              />
            </label>
            <button type="submit" class="secondary" disabled={matcher.releaseLoading}>
              {matcher.releaseLoading ? 'loading…' : 'load exact release'}
            </button>
          </form>
          {#if releaseIdError}
            <p class="err release-id-error">{releaseIdError}</p>
          {/if}
            ...
            <div class="release">
              ...
              {#if status}
                <div class="status" class:ok={status.tone === 'ok'} class:warn={status.tone === 'warn'}>
                  ...
                  <!-- The way out belongs where the problem is announced. The
                       same button lives down in the files section, but by the
                       time you scroll to it you have already ground through a
                       picker row per duplicate. -->
                  {#if duplicateFiles.length > 0 && preview && preview.file_count > preview.total_tracks}
                    <button class="secondary status-fix" onclick={confirmDeleteDuplicates}>
                      delete {duplicateFiles.length} duplicate{duplicateFiles.length === 1 ? '' : 's'}
                    </button>
                  {/if}
                </div>
              {/if}
              ...
              {#if identityConflict}
                <p class="err match-hint"><strong>Apply blocked.</strong> {identityConflict}</p>
              {/if}

              {#if missingReleaseTracks.length > 0}
                <p class="err match-hint">
                  <strong>Apply blocked. Missing from this download:</strong>
                  {missingReleaseTracks.map(({ track }) => `${track.disc}-${track.track.toString().padStart(2, '0')} ${track.title}`).join(' · ')}
                </p>
              {/if}
              ...
              {#if match.existingAlbum}
                ...
                <div class="dup-warn">
                  ...
                  <button
                    ...
                    disabled={match.applying || identityConflict !== null}
                  >
...
<style>
  ...
  .close:hover { background: var(--hairline); color: var(--espresso); }
  .head { flex: 1; }
  .label {
    font-size: 0.68rem;
  ...
  /* margin: 0 here would override the .inbox > * auto-centering and pin the
     list flush-left while the header stays centered — keep the sides auto. */
  .folders { list-style: none; padding: 0; margin: 0 auto; }
  .folders li {
    display: flex;
    align-items: stretch;
    gap: 0.3rem;
  }
  .folders li.kbd-selected .folder {
    border-color: var(--brick);
    box-shadow: inset 3px 0 0 var(--brick);
  }
  .singles-sweep { margin-bottom: 0.6rem; }
  .f-single {
    align-self: center;
    font: inherit;
    font-size: 0.72rem;
  ...
  .f-single:hover:not(:disabled) { border-color: var(--brick); color: var(--brick); }
  .f-single:disabled { opacity: 0.4; cursor: default; }
  .folder {
    flex: 1;
    text-align: left;
    background: var(--surface);
  ...
  .cmp-complete { color: var(--ok-fg); border-color: var(--ok-border); }
  ...
  .fp-ready { color: var(--ok-fg); border-color: var(--ok-border); }
  ...
  .row-conflict td { background: var(--brick-wash); }
  ...
  .files li > .fn { grid-column: 3; }
  .files li > .ft { grid-column: 3; }
  .file-single {
    grid-row: 1 / span 3;
    grid-column: 2;
    align-self: center;
  ...
  .file-single:hover:not(:disabled) { border-color: var(--brick); color: var(--brick); }
  .file-single:disabled { opacity: 0.4; cursor: default; }
  .files li > .read-err { grid-column: 3; }
  ...
  .file-del:hover { color: var(--err-fg); background: color-mix(in srgb, var(--err-fg) 8%, transparent); }
  .fn { font-weight: 500; }
  ...
  .release-id-search label {
    display: flex;
    flex-direction: column;
    gap: 0.2rem;
    min-width: 0;
  }
  .release-id-search label span {
    font-size: 0.65rem;
    text-transform: uppercase;
    letter-spacing: 0.1em;
    color: var(--faded);
  }
  .release-id-search input {
    background: var(--paper);
    border: 1px solid var(--hairline);
    border-radius: var(--radius-md);
    padding: 0.4rem 0.6rem;
  ...
  .release-id-search input:focus { outline: none; border-color: var(--brick); }
  .release-id-search .secondary { align-self: end; padding: 0.45rem 0.9rem; }
  .release-id-error { margin-top: -0.55rem; }
  ...
  /* Push the fix to the trailing edge so the banner reads
     problem → detail → button. */
  .status-fix { margin-left: auto; flex: 0 0 auto; align-self: center; }
  ...
  .conf-ok { color: var(--ok-fg); border-color: var(--ok-border); }
  .conf-warn { color: var(--warn-fg); border-color: var(--warn-border); }
