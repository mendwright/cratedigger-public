<script lang="ts">
  ...
  // Shelf state: which of the four discovery rails is expanded. Persisted
  // per rail in localStorage so closing/reopening Cratedigger remembers.
  // Counts are bound from the rail components — they always render (so the
  // count stays current) but only show their row when the corresponding
  // shelf pill is toggled on.
  const SHELF_KEYS = {
  ...
  function readShelf(key: string): boolean {
    return typeof window !== 'undefined' && localStorage.getItem(key) === '1'
  }
  function writeShelf(key: string, val: boolean): void {
    try { localStorage.setItem(key, val ? '1' : '0') } catch { /* noop */ }
  }
  let shelfRecent = $state(readShelf(SHELF_KEYS.recent))
  let shelfNeglected = $state(readShelf(SHELF_KEYS.neglected))
  let shelfFiveStar = $state(readShelf(SHELF_KEYS.fivestar))
  let shelfNeverSpun = $state(readShelf(SHELF_KEYS.neverspun))
  let countRecent = $state(0)
  let countNeglected = $state(0)
  let countFiveStar = $state(0)
  let countNeverSpun = $state(0)
  function togRecent(): void { shelfRecent = !shelfRecent; writeShelf(SHELF_KEYS.recent, shelfRecent) }
  function togNeglected(): void { shelfNeglected = !shelfNeglected; writeShelf(SHELF_KEYS.neglected, shelfNeglected) }
  function togFiveStar(): void { shelfFiveStar = !shelfFiveStar; writeShelf(SHELF_KEYS.fivestar, shelfFiveStar) }
  function togNeverSpun(): void { shelfNeverSpun = !shelfNeverSpun; writeShelf(SHELF_KEYS.neverspun, shelfNeverSpun) }
  ...
  const albums = $derived.by(() => {
    ...
    const filtered = source.filter((a) => albumMatchesFilters(a, filters, labelMap[a.ratingKey] ?? [], canonical))
  ...
  {#if searchActive}
    ...
    {:else if searchResults}
      ...
      {#if searchResults.albums.length > 0}
        ...
        <ul class="grid" style="--cover-size: {coverSize}px;">
  ...
  {:else}
    ...
    {:else}
      {#if !anyFilterActive}
        ...
        <div class="shelf" role="group" aria-label="Discovery shelves">
          <button
            class="shelf-pill"
            class:shelf-pill-open={shelfRecent}
            onclick={togRecent}
            aria-expanded={shelfRecent}
            disabled={countRecent === 0}
          >
            <span class="chev">{shelfRecent ? '▾' : '▸'}</span>
            <span class="label">recently played</span>
            <span class="count">{countRecent}</span>
          </button>
          <button
            class="shelf-pill"
            class:shelf-pill-open={shelfNeglected}
            onclick={togNeglected}
            aria-expanded={shelfNeglected}
            disabled={countNeglected === 0}
          >
            <span class="chev">{shelfNeglected ? '▾' : '▸'}</span>
            <span class="label">neglected gems</span>
            <span class="count">{countNeglected}</span>
          </button>
          <button
            class="shelf-pill"
            class:shelf-pill-open={shelfFiveStar}
            onclick={togFiveStar}
            aria-expanded={shelfFiveStar}
            disabled={countFiveStar === 0}
          >
            <span class="chev">{shelfFiveStar ? '▾' : '▸'}</span>
            <span class="label">five-star vault</span>
            <span class="count">{countFiveStar}</span>
          </button>
          <button
            class="shelf-pill"
            class:shelf-pill-open={shelfNeverSpun}
            onclick={togNeverSpun}
            aria-expanded={shelfNeverSpun}
            disabled={countNeverSpun === 0}
          >
            <span class="chev">{shelfNeverSpun ? '▾' : '▸'}</span>
            <span class="label">never spun</span>
            <span class="count">{countNeverSpun}</span>
          </button>
        </div>

        <!--
          The four rails always render so they compute their counts even
          when collapsed. hideHeading=true keeps the shelf as the sole
          interactive surface; expanded={state} lets the pill control
          whether the row of albums shows up — directly under the pills.
        -->
        <RecentAlbumsRail
          hideHeading
          expanded={shelfRecent}
...
<style>
  ...
  .shelf-pill-open:hover { background: var(--walnut); border-color: var(--walnut); }
  .shelf-pill:disabled { opacity: 0.4; cursor: default; }
  .shelf-pill .chev {
  ...
  .left, .right { display: flex; gap: 0.6rem; align-items: center; }
  .right { flex-wrap: wrap; justify-content: flex-end; }
  .left { flex-direction: column; align-items: flex-start; gap: 0.15rem; flex-shrink: 0; }
  .search-wrap {
  ...
  select, button, input, .size-control {
    -webkit-app-region: no-drag;
  }
  select {
    background: var(--inset);
