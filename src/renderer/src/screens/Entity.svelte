<script lang="ts">
  ...
  const missing = $derived(plexState.entityPage.missing)
  const missingLoading = $derived(plexState.entityPage.missingLoading)

  // Cover Art Archive front covers 404 for plenty of release-groups; remember
  // the failures so the fallback letter renders instead of a broken image.
  let missingCoverFailed = $state<Record<string, true>>({})

  function onMissingClick(m: MissingRelease): void {
    // Capture the name first — openSlskd() tears down the entity page state,
    // which nulls the `entity` derived before the next line reads it.
    const artistName = entity?.name
    if (PUBLIC_BUILD || !artistName) {
      window.open(`https://musicbrainz.org/release-group/${m.mbid}`, '_blank')
      return
    }
    // Land on the Soulseek tab with the search already running. Straight to
    // results — no release picker; artist/title are exact from MB here,
    // unlike the fuzzy scraped queries the picker flow exists for.
    void plexState.openSlskd()
    void plexState.slskd.runSearch(artistName, m.title)
  }
  ...
  const mbid = $derived(plexState.entityPage.mbid)
  ...
  // Four modes used to stack in one scroll: their own albums, gaps in the
  // discography, role credits, credited-on works, and the bio. Tabs instead.
  //
  // Held as {key, tab} and derived rather than reset by an $effect — same
  // reason as AlbumDetail: an effect that writes the tab while reading state
  // the template renders can wedge the component, and the entity object is
  // replaced as each lookup lands (bio, browse, credits).
  type ETabId = 'albums' | 'appears' | 'credits' | 'bio'
  const ETABS: { id: ETabId; label: string }[] = [
    { id: 'albums', label: 'Albums' },
    { id: 'appears', label: 'Appears on' },
    { id: 'credits', label: 'Credits' },
    { id: 'bio', label: 'Bio' }
  ]
  let etabChoice = $state<{ key: string | null; tab: ETabId }>({ key: null, tab: 'albums' })
  // Keyed on the name, not the MBID: `browse` resolves a second or two after
  // the page opens, so an mbid-based key would flip once on arrival and bounce
  // you back to Albums while you were reading. Two artists sharing a name just
  // means the tab doesn't reset, which costs nothing.
  const etabKey = $derived(entity?.name ?? null)
  const etab = $derived(etabChoice.key === etabKey ? etabChoice.tab : 'albums')
  function setEtab(id: ETabId): void {
    etabChoice = { key: etabKey, tab: id }
  }
  ...
  // One tab per source that has usable text: Plex's curated summary plus the
  // Last.fm / Wikipedia variants from the bio fetch. Plex's agent scrapes
  // Last.fm artist blurbs (footer intact) into the summary field — those fold
  // into the Last.fm tab (longer copy wins) rather than posing as Plex.
  type BioTab = { source: 'plex' | 'wikipedia' | 'lastfm'; text: string; url: string | null }
  const bioTabs = $derived.by((): BioTab[] => {
    const rawPlex = browse?.plexArtist?.summary?.trim() ?? ''
    const plexIsScrape = isLastfmScrape(rawPlex)
    const cleanedPlex = plexIsScrape ? stripLastfmFooter(rawPlex) : rawPlex
    const fromPlex = isAmbiguousArtistBio(cleanedPlex) ? '' : cleanedPlex
    const variants = (bio?.variants ?? (bio?.text ? [bio] : [])).filter(
      (v) => v.text.trim() && !isAmbiguousArtistBio(v.text.trim())
    )
    const wiki = variants.find((v) => v.source === 'wikipedia') ?? null
    const lfm = variants.find((v) => v.source === 'lastfm') ?? null
    const scrapeText = plexIsScrape ? fromPlex : ''
    const lfmText = lfm?.text.trim() ?? ''
    const tabs: BioTab[] = []
    if (fromPlex && !plexIsScrape) tabs.push({ source: 'plex', text: fromPlex, url: null })
    if (wiki) tabs.push({ source: 'wikipedia', text: wiki.text.trim(), url: wiki.url })
    if (lfmText || scrapeText) {
      tabs.push({
        source: 'lastfm',
        text: lfmText.length >= scrapeText.length ? lfmText : scrapeText,
        url: lfm?.url ?? null
      })
    }
    // With no Plex tab, lead with the bio fetch's longer-wins pick.
    if (tabs[0]?.source !== 'plex' && bio?.source === 'lastfm') {
      tabs.sort((a, b) => (a.source === 'lastfm' ? -1 : b.source === 'lastfm' ? 1 : 0))
    }
    return tabs
  })

  // Sticky selection: whichever tab renders first stays until the user picks —
  // sources that arrive later add a tab instead of swapping the text mid-read.
  let activeBioTab = $state<BioTab['source'] | null>(null)
  $effect(() => {
    mbid
    activeBioTab = null
    bioExpanded = false
  ...
  $effect(() => {
    if (activeBioTab === null && bioTabs.length > 0) activeBioTab = bioTabs[0].source
  })
  const currentBioTab = $derived(
    bioTabs.find((t) => t.source === activeBioTab) ?? bioTabs[0] ?? null
  )
  const bioText = $derived(currentBioTab?.text ?? '')
  ...
  const bioSource = $derived<'plex' | 'wikipedia' | 'lastfm' | null>(
    currentBioTab?.source ?? null
  )
  const bioUrl = $derived(currentBioTab?.url ?? null)

  function pickBioTab(source: BioTab['source']): void {
    activeBioTab = source
    bioExpanded = false
  }
...
<section data-view="artist" onscroll={onScroll}>
  ...
  {:else if entity}
    <div class="hero" data-artist-hero class:has-image={!!heroImage}>
      {#if heroImage}
      ...
      <div class="hero-content">
        ...
        <div class="hero-meta">
          ...
          <h1 data-artist-title>{entity.name}</h1>
          <div class="meta">
          ...
          {#if bio?.tags && bio.tags.length > 0}
            <div class="tags">
              {#each bio.tags.slice(0, 8) as tag (tag)}
                <button
                  class="tag"
                  onclick={() => plexState.openGenreEntity(tag)}
                  title={`all ${tag} albums in your library`}>{tag}</button>
              {/each}
            </div>
          {/if}
    ...
    <div class="body">
      <nav class="etabs" data-artist-tabs aria-label="Artist sections">
        {#each ETABS as t (t.id)}
          <button
            class="etab"
            class:etab-active={etab === t.id}
            aria-current={etab === t.id ? 'true' : undefined}
            onclick={() => setEtab(t.id)}
          >{t.label}</button>
        {/each}
      </nav>
      ...
      {#if etab === 'albums'}
        ...
        {#if albums.length > 0}
          <h2 data-section-head><span>albums on your server</span><span class="count">{albums.length}</span></h2>
          <ul class="grid" data-grid="albums">
            {#each albums as album (album.ratingKey)}
              ...
              <li>
                <button class="card" data-card="album" onclick={() => openAlbum(album.ratingKey)}>
                  <div class="cover">
                    {#if src}
        ...
        {#if missing && missing.length > 0}
          <h2 data-section-head><span>not on your server</span><span class="count">{missing.length}</span></h2>
          <p class="hint deep-cuts-hint">
            {PUBLIC_BUILD ? 'from the musicbrainz discography — click to view' : 'from the musicbrainz discography · click one to search soulseek'}
          </p>
          <ul class="grid" data-grid="albums">
            {#each missing as m (m.mbid)}
              <li>
                <button class="card" data-card="album" onclick={() => onMissingClick(m)} title={m.title}>
                  ...
                  <div class="artist">
                    {m.year ?? 'date unknown'}{m.primaryType === 'EP' ? ' · EP' : ''}
                  </div>
                </button>
              </li>
            {/each}
          </ul>
        {:else if missingLoading && browse}
          <p class="hint pulse-hint"><span class="dotpulse"></span> checking the discography for gaps…</p>
        {/if}

      {:else if etab === 'appears'}
        ...
        {:else if credited}
          {#if deepCuts.length > 0}
            <h2 data-section-head><span>deep cuts</span><span class="count">{deepCuts.length}</span></h2>
            ...
            <ul class="grid" data-grid="albums">
              {#each deepCuts as entry (entry.album.ratingKey)}
                ...
                <li>
                  <button class="card" data-card="album" onclick={() => openAlbum(entry.album.ratingKey)}>
                    <div class="cover">
                      {#if src}
          ...
          {#if credited.inLibrary.length > 0}
            <h2 data-section-head><span>credited on — in your library</span><span class="count">{credited.inLibrary.length}</span></h2>
            <ul class="grid" data-grid="albums">
          ...
          {#if credited.notInLibrary.length > 0}
            <h2 data-section-head><span>credited on — not in your library</span><span class="count">{credited.notInLibrary.length}</span></h2>
      ...
      {:else if etab === 'credits'}
        {#if appearanceGroups.length > 0}
          <h2 data-section-head><span>your library credits</span><span class="count">{appearanceGroups.length} {appearanceGroups.length === 1 ? 'role' : 'roles'}</span></h2>
        ...
        {#if appearanceGroups.length === 0}
          <p class="hint">no role credits yet — they build up as you open albums.</p>
        {/if}
      {:else}
        {#if bioText}
          ...
          <div class="bio-block">
            {#if bioTabs.length > 1}
              <div class="bio-tabs">
                {#each bioTabs as t (t.source)}
                  <button
                    class="bio-tab"
                    class:active={t.source === bioSource}
                    onclick={() => pickBioTab(t.source)}
                  >{t.source === 'plex' ? 'Plex' : t.source === 'wikipedia' ? 'Wikipedia' : 'Last.fm'}</button>
                {/each}
              </div>
            {/if}
        ...
        {#if !bioText && !bioLoading}
          <p class="hint">no bio found for this artist.</p>
        {/if}
      {/if}
...
<style>
  ...
  .condensed {
    ...
    /* Fixed, not tokenised: this floats over the artist hero image, which no
       theme colours, and has to stay legible over a bright photo. */
    ...
    /* NOT draggable, and this one is the whole ballgame for the recurring
       "top bar goes dead" bug. Chromium collects draggable rects in *visual*
       space (LocalToAbsoluteQuad applies transforms), so while this header is
       "hidden" at translateY(-100%) its drag rect is parked one header-height
       above its flow position — i.e. directly on top of TopToolbar, full
       width. `pointer-events: none` is no defense: native drag hit-testing
       ignores it entirely. Painting after the toolbar, its `drag` overrode the
       toolbar's `no-drag`, so every control up there silently became a window
       drag handle until a reload re-emitted the rects. Dragging lives on the
       toolbar's one fixed handle — see TopToolbar's .drag-fill. */
  ...
  .top-nav {
    ...
    /* Not draggable: absolutely positioned inside the scrolling hero, so it
       travels up and under the toolbar as you scroll — a moving drag rect,
       the exact thing that goes stale. See .condensed above. */
    -webkit-app-region: no-drag;
    pointer-events: none;
  }
  .back {
    ...
    align-items: center;
    gap: 0.4rem;
    /* Same as .condensed — this sits on the hero image. */
  ...
  .hero {
    ...
    overflow: hidden;
    background: linear-gradient(180deg, var(--surface-deep) 0%, var(--paper) 100%);
    isolation: isolate;
  }
  ...
  .hero-fade {
    ...
    background:
      ...
      radial-gradient(ellipse at 30% 20%, color-mix(in srgb, var(--brick) 6%, transparent), transparent 55%),
  ...
  .hero-portrait {
    ...
    overflow: hidden;
    box-shadow:
      0 24px 60px rgba(var(--shadow-color), 0.3),
  ...
  h1 {
    ...
    line-height: 1;
    margin: 0;
    text-shadow: 0 2px 28px rgba(var(--shadow-color), 0.28);
  ...
  .plex-pill.found {
    ...
    border-color: color-mix(in srgb, var(--ok-fg) 28%, transparent);
  }
  ...
  .pulse.on {
    ...
    box-shadow: 0 0 0 0 color-mix(in srgb, var(--ok-fg) 60%, transparent);
    animation: pulse 1.6s ease-out infinite;
  }
  @keyframes pulse {
    0%   { box-shadow: 0 0 0 0 color-mix(in srgb, var(--ok-fg) 55%, transparent); }
  ...
  .bio-tab:hover { background: var(--ink-tint-08); color: var(--espresso); }
  .bio-tab.active {
  ...
  .cover {
    ...
    overflow: hidden;
    box-shadow: 0 6px 18px rgba(var(--shadow-color), 0.22);
    transition: box-shadow 220ms ease;
  }
  .card:hover .cover {
    box-shadow:
      0 14px 32px rgba(var(--shadow-color), 0.28),
      0 0 0 1px var(--ink-tint-04);
  }
  .cover img { width: 100%; height: 100%; object-fit: cover; display: block; }
  /* Not-owned covers read muted until hover, with the action pill riding on top. */
  ...
  .missing-action {
    ...
    white-space: nowrap;
    background: rgba(var(--shadow-color), 0.85);
  ...
  .dotpulse {
    ...
    border-radius: 50%;
    background: var(--brick);
    box-shadow: 0 0 8px color-mix(in srgb, var(--brick) 50%, transparent);
