...
<section onscroll={onScroll}>
  ...
  {:else if entity}
    ...
    <div class="body">
      <nav class="etabs" aria-label="Artist sections">
      ...
      {#if etab === 'albums'}
        ...
        {#if missing && missing.length > 0}
          <h2><span>not on your server</span><span class="count">{missing.length}</span></h2>
          ...
          <ul class="grid">
            {#each missing as m (m.mbid)}
              <li>
                <button class="card" onclick={() => onMissingClick(m)} title={m.title}>
