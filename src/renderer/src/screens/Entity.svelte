<script lang="ts">
  ...
  const visibleBio = $derived(
    bioNeedsTruncation && !bioExpanded ? bioText.slice(0, 720).trimEnd() + '…' : bioText
  )
...
<style>
  ...
  .meta { color: var(--walnut); font-size: 0.92rem; margin-top: 0.85rem; display: flex; align-items: center; flex-wrap: wrap; }
  .meta .disambig { color: var(--faded); font-style: italic; }
