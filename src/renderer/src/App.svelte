<script lang="ts">
  ...
  function onGlobalKey(e: KeyboardEvent): void {
    ...
    // Cmd+1..9: switch top-level views. Order mirrors the toolbar.
    if (cmd && !e.shiftKey && !e.altKey && /^[1-9]$/.test(e.key)) {
      if (inText) return
      e.preventDefault()
      switch (e.key) {
        case '1': plexState.goHome(); break
        case '2': if (!PUBLIC_BUILD) void plexState.openInbox(); break
        case '3': if (!PUBLIC_BUILD) void plexState.openTriage(); break
        case '4': if (!PUBLIC_BUILD) void plexState.openSlskd(); break
        case '5': void plexState.openNewReleases(); break
        case '6': void plexState.openShows(); break
        case '7': void plexState.openListenLater(); break
        case '8': void plexState.openCharts(); break
        case '9': if (!PUBLIC_BUILD) void plexState.openArtFixer(); break
      }
      return
    }
  }
...
<style>
  ...
  .shows-banner-main:hover { background: var(--hover, rgba(0, 0, 0, 0.04)); }
