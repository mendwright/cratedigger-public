...
// Last.fm sometimes returns a directory-style wiki for a shared artist name
// instead of the MBID-specific artist. Showing all of those numbered bios on
// one entity page is actively misleading, so treat the tell-tale opener as no
// bio and let the anchored Wikipedia lookup win instead.
const AMBIGUOUS_ARTIST_BIO_RE =
  /^\s*there (?:are|is) (?:at least )?(?:(?:\d+|multiple|several|many|more than one|two|three|four|five|six|seven|eight|nine|ten)(?: \(\d+\))? )?(?:artists?|bands?|groups?|acts?) (?:(?:with|using|who use|sharing) (?:the |this )?(?:same )?name\b|(?:called|named)\b)/i

export function isAmbiguousArtistBio(text: string | null | undefined): boolean {
  return !!text && AMBIGUOUS_ARTIST_BIO_RE.test(text)
}
