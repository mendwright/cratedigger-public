...
export interface AlbumCredits {
  ...
  // The release's first credited artist — lets the artist-name click open the
  // entity page with zero network. Undefined on legacy cache entries.
...
export interface GenreIndexProgress {
  running: boolean
  done: number
  total: number
  current: string | null
...
// One release-group from a record label's MB catalog, carrying its artist
// credit (unlike MbArtistReleaseGroup, where the artist is implicit).
export interface LabelReleaseGroup {
  rgMbid: string
  title: string
  artist: string
  firstReleaseDate: string | null
...
// A record label's MB catalog, resolved from a label *name* (the label index
// stores names, not MBIDs). `truncated` = the label has more releases than
// the page cap; the browse stopped early.
export interface LabelDiscography {
  labelMbid: string
  labelName: string
...
export interface LyricsResult {
  ...
  source: 'lrclib' | 'genius' | 'bandcamp'
  // Where to find the canonical record (lrclib.net page so users can correct
  // mismatches, or the Genius/Bandcamp song page).
  url: string | null
}
...
export interface ArtistBio extends Bio {
  ...
  tags: string[]
  /** Every source that returned usable text, for the artist page's source
   *  tabs. The top-level text/source/url remain the longer-wins pick. */
  variants: Bio[]
}
...
// How radio chooses what follows: 'drift' follows the vibe of what's playing,
// the lock modes constrain the pool to the current album's label/genre/era,
// 'surprise' ignores affinity and digs up unplayed corners of the library.
export type RadioSteer = 'drift' | 'label' | 'genre' | 'era' | 'surprise'

// One persisted radio pick — the long-term memory that keeps radio from
// re-playing an album for weeks and the same artist night after night.
...
// the user's shelf (only when it fits the current vibe), 'label-mate' =
// shares a record label, 'similar' = via Plex's similar artists, 'library' =
// genre/era neighbor from the wider library, 'same-artist' = another record
// by the same artist, 'surprise' = the surprise steer's wildcard.
export interface ContinuationPick {
  albumRatingKey: string
  title: string
  artist: string
  artistRatingKey?: string | null
  source: 'listen-later' | 'similar' | 'same-artist' | 'label-mate' | 'library' | 'surprise'
  // Human context for the queue divider ("Soul Jazz Records", a genre name).
  via?: string | null
}
