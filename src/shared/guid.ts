...
/**
 * Accept a bare release MBID or a copied musicbrainz.org/release URL.
 * Other MusicBrainz entity URLs are deliberately rejected.
 */
export function parseMusicBrainzReleaseInput(input: string): string | null {
  const value = input.trim()
  if (isValidMbid(value)) return value.toLowerCase()

  try {
    const url = new URL(value)
    if (url.hostname !== 'musicbrainz.org' && url.hostname !== 'www.musicbrainz.org') {
      return null
    }
    const match = /^\/release\/([0-9a-f-]{36})\/?$/i.exec(url.pathname)
    return match && isValidMbid(match[1]) ? match[1].toLowerCase() : null
  } catch {
    return null
  }
}
 ...
 * when no explicit tag is present — `explicit` says whether the tag was
 * really there. Plex's MusicBrainz agent writes untagged `mbid://UUID` on
 * every item type, so on an artist item an untagged GUID is the artist's
 * MBID; callers that know the item type should trust that over the default.
 */
export function extractMbidFromGuid(
  guid: string
): { entity: 'release' | 'release-group' | 'artist'; mbid: string; explicit: boolean } | null {
  const m = MBID_GUID_RE.exec(guid)
  if (!m) return null
  const entity = (m[1] ?? 'release') as 'release' | 'release-group' | 'artist'
  return { entity, mbid: m[2].toLowerCase(), explicit: m[1] !== undefined }
}
