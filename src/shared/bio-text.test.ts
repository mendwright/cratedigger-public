...
const AMBIGUOUS_ARTIST_BIO =
  'There are at least 11 artists with the name Goat. 1. With an unfettered sound influenced by everything from Afro-beat to Anatolian funk, Goat are an experimental psychedelic rock band based out of Gothenburg, Sweden. 2. Chris Gates, better known as goat, is a video game remixer.'

describe('isAmbiguousArtistBio', () => {
  it('rejects Last.fm directory pages for several same-named artists', () => {
    expect(isAmbiguousArtistBio(AMBIGUOUS_ARTIST_BIO)).toBe(true)
    expect(isAmbiguousArtistBio('There are multiple artists with this name:')).toBe(true)
    expect(isAmbiguousArtistBio('There is more than one artist with the same name.')).toBe(true)
    expect(
      isAmbiguousArtistBio(
        'There are three (3) bands called Sluice. 1) Indie folk project from North Carolina led by songwriter Justin Morris. 2) Sluice is an independent rock band from just south of Sydney Australia.'
      )
    ).toBe(true)
    expect(isAmbiguousArtistBio('There are two groups named Suede.')).toBe(true)
  })

  it('keeps a bio that describes one artist', () => {
    expect(isAmbiguousArtistBio(CURATED)).toBe(false)
    expect(isAmbiguousArtistBio('Goat are a Swedish experimental fusion group.')).toBe(false)
  })
})
