// MUST stay the first import: it redirects userData for dev builds, and
// ./store.js resolves that path the moment it is imported.
import './user-data-dir.js'
...
import { getValue, migrateSecrets, migrateAccounts } from './store.js'
...
import { startBijouGrabNotifier, stopBijouGrabNotifier } from './bijou-notify.js'
import { startSlskdRetrySweeper, stopSlskdRetrySweeper } from './slskd-retry.js'
import { detachInstallerDmg } from './dmg-cleanup.js'
import { startTheaterServer } from './theater-server.js'
import { closeClassicalDatabase, initializeClassicalDatabase } from './classical/service.js'
...
// Before ready — macOS delivers a launch-via-link's 'open-url' early.
initDeepLinks(
  () => mainWindow,
  () => createWindow()
)
...
// The primary library window (not the mini player) — deep links target it.
...
function createWindow(): void {
  const win = new BrowserWindow({
    ...
    // Center the traffic lights in the renderer's top toolbar row (which
    // reserves 84px of left padding for them).
...
app.whenReady().then(() => {
  ...
  seedSecretsFromFile()
  registerImageProtocolHandler()
  void initializeClassicalDatabase().catch((err) => {
    console.error('[classical] database initialization failed:', err)
  })
  registerIpc()
  initAutoUpdater()
  detachInstallerDmg()
  startTheaterServer()
  startBijouGrabNotifier()
  startSlskdRetrySweeper()
  createWindow()
  app.on('activate', () => {
...
app.on('before-quit', () => {
  stopMediaKeys()
  stopBijouGrabNotifier()
  stopSlskdRetrySweeper()
  closeClassicalDatabase()
})
