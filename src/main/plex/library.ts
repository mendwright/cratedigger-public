...
function toTrack(t: RawTrack): PlexTrack {
  return {
    ...
    guids: (t.Guid ?? []).map((g) => g.id),
    filePath: t.Media?.flatMap((m) => m.Part ?? []).find((p) => p.file)?.file ?? null
  }
}
...
export async function getAlbumDetail(ctx: ServerContext, ratingKey: string): Promise<AlbumDetail> {
  const [albumRes, tracksRes] = await Promise.all([
    ...
    plexFetch(`${ctx.baseUrl}/library/metadata/${ratingKey}/children?includeGuids=1&includeMedia=1`, { headers: headers(ctx) })
  ])
...
export async function listArtistAlbums(
  ...
  // Plex's /children on an artist silently omits anything not tagged
  // Format=Album — EPs, singles, and rarities all vanish (Boards of Canada
  // showed 6 of 15). The artist-scoped section query returns the full
  // discography, so resolve the artist's section and query that instead.
  const metaRes = await plexFetch(`${ctx.baseUrl}/library/metadata/${artistRatingKey}`, {
    headers: headers(ctx)
  })
  if (!metaRes.ok) throw new Error(`Artist fetch failed: ${metaRes.status}`)
  const metaJson = (await metaRes.json()) as MediaContainer<{ librarySectionID?: number }>
  const sectionId = metaJson.MediaContainer.Metadata?.[0]?.librarySectionID
  if (sectionId == null) throw new Error('Artist has no library section')
  const albums: PlexAlbum[] = []
  const pageSize = 500
  let offset = 0
  for (;;) {
    const url = `${ctx.baseUrl}/library/sections/${sectionId}/all?type=9&artist.id=${artistRatingKey}&includeMeta=0&includeGuids=1`
    const res = await plexFetch(url, {
      headers: headers(ctx, {
        'X-Plex-Container-Start': String(offset),
...
// These GUIDs come off an *artist* item, so an untagged `mbid://UUID` — which
// is what Plex's MusicBrainz agent actually writes on nearly every artist —
// is the artist's MBID. Only an explicit release/release-group tag rules a
// GUID out. Requiring the `artist/` tag here left Release Radar with zero
// artists to walk for three months and an eternal "Nothing new — yet."
export function extractArtistMbid(guids: RawGuid[]): string | null {
  for (const g of guids) {
    const parsed = extractMbidFromGuid(g.id)
    if (!parsed) continue
    if (parsed.entity === 'artist' || !parsed.explicit) return parsed.mbid
  }
  return null
}
...
// One artist's MBID straight from its Plex metadata — a single LAN call, so
// an artist click can open the entity page without a MusicBrainz name search
// (paced at ~1/s behind whatever else is in the MB queue).
export async function getArtistMbid(ctx: ServerContext, ratingKey: string): Promise<string | null> {
  const res = await plexFetch(`${ctx.baseUrl}/library/metadata/${encodeURIComponent(ratingKey)}?includeGuids=1`, {
    headers: headers(ctx)
  })
  if (!res.ok) return null
  const json = (await res.json()) as MediaContainer<{ type?: string; Guid?: RawGuid[] }>
  const item = json.MediaContainer.Metadata?.[0]
  if (!item || item.type !== 'artist') return null
  return extractArtistMbid(item.Guid ?? [])
}
