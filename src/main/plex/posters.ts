...
/**
 * Upload raw image bytes as a new poster and select it. Used for local uploads —
 * a file Cratedigger serves over cratedigger-cover:// that Plex can't fetch by
 * URL, so we POST the bytes directly. Plex stores it as a `local` poster and
 * auto-selects it, same as the remote-URL path above.
 */
export async function uploadAlbumPosterData(
  ctx: ServerContext,
  rk: string,
  data: Uint8Array,
  contentType: string
  ...
  const res = await plexFetch(url, {
    method: 'POST',
    headers: { ...headers(ctx), 'Content-Type': contentType },
    body: data as unknown as RequestInit['body']
  })
  if (res.status === 401)
    throw new Error('poster upload failed: 401 — add a Plex admin token in Settings (or sign in as the server owner) to edit cover art')
  if (!res.ok) throw new Error(`poster upload failed: ${res.status}`)
}
