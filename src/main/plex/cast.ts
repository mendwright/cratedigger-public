...
export function chooseRadioAlbum(args: RadioChoiceArgs): ContinuationPick | null {
  ...
  for (const c of args.candidates) {
    ...
    // Whole-library candidates must earn entry with real affinity. Era alone
    // is not affinity — Blood Sugar Sex Magik is "within 7 years" of
    // Philophobia, and its 10.0 critic rating once carried it into the top
    // picks on that alone. Never again.
    if (c.origin === 'library' && steer === 'drift' && !sharedGenre(c.album)) continue
    eligible.push(c)
  }
  ...
  const score = (c: RadioCandidate): number => {
    const a = c.album
    // Plex's `rating` is the imported critic/audience score, not the user's —
    // capped to a tiebreak so canonical mainstream albums can't out-shout the
    // vibe signals (the +rating/2 era shipped exactly one pick: RHCP after
    // Arab Strap).
    const ratingNudge = a.rating ? Math.min(1, a.rating / 10) : 0
    if (steer === 'surprise') {
      // Affinity is the point of the other steers; surprise wants fresh digs.
      return novelty(a) * 2 + ratingNudge
    }
    let s = 0
    if (sharedGenre(a)) s += 3
    if (eraClose(a, 7)) s += 2
    if (sharedLabel(a)) s += 2
    // Plex's similar-artists graph is the strongest signal we have (it had
    // Low/Codeine/Mogwai ready for Arab Strap) — let it dominate.
    if (c.origin === 'similar') s += 4
