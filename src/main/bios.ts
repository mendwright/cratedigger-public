...
const WIKI_USER_AGENT = 'cratedigger/0.4.0 (https://github.com/jaybeaman/cratedigger; contact via github)'
...
// v3: v1 entries were cached when a Last.fm one-liner blocked the Wikipedia
// fetch entirely; v2 entries predate stub-lead expansion (History/Career
// sections) and the guarded title search. Bump so both refetch.
const artistBioCache = new JsonCache<ArtistBio>('artist-bios-v3')
...
export async function getArtistBio(opts: {
  ...
  const lfmText = lfm ? stripLastfmFooter(lfm.bio?.content || lfm.bio?.summary || '') : ''
  if (lfm) {
