...
export async function runReleaseRadar(serverId: string): Promise<ReleaseRadarSnapshot> {
  const result = await job.run(
    ...
    async () => {
      ...
      if (job.cancelled) {
        // A cancelled walk only covered part of the library. Keep whatever
        // snapshot was already on disk rather than replacing it with a
        // partial (often empty) one; the per-artist cache still keeps the
        // progress made, so the next run picks up where this one stopped.
        const kept = getValue('releaseRadar') ?? { recent: [], upcoming: [], builtAt: null }
        job.update({ phase: 'done', currentArtist: null, snapshot: kept })
        return kept
      }
