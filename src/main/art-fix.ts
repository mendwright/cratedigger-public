...
// Write a cover the user picked in the "choose cover" modal into Plex itself, so
// every client repaints — external headless-Plexamp casts and the living-room
// Pi display included — not just Cratedigger's local thumbnail. Shares the Art
// Fixer's owner-token write path (writeCtx) and its revert log, so a mistaken
// pick is undoable from the Art Fixer's "revert last" exactly like a scan fix.
// Best-effort by design: the caller keeps the local override even when this
// throws (e.g. no owner token → 401) and surfaces the reason to the user.
export async function pushCoverToPlex(args: {
  serverId: string
  rk: string
  // Free-form provenance for the revert log ('itunes' | 'caa' | 'local' | …).
  source: string
  // Exactly one of these: a public URL Plex fetches itself, or raw image bytes
  // for a local upload Plex can't reach by URL.
  remoteUrl?: string
  localData?: { bytes: Uint8Array; contentType: string }
}): Promise<void> {
  const ctx = await resolveServerContext(args.serverId)
  const current = (await listAlbumPosters(ctx, args.rk)).find((p) => p.selected) ?? null

  // Log BEFORE writing so a crash mid-write still leaves a revertable record.
  appendArtFixLog({
    rk: args.rk,
    prevRatingKey: current?.ratingKey ?? null,
    newUrl: args.remoteUrl ?? null,
    newRatingKey: null,
    source: args.source,
    at: Date.now()
  })
  ...
  const wctx = writeCtx(ctx)
  if (args.localData) {
    await uploadAlbumPosterData(wctx, args.rk, args.localData.bytes, args.localData.contentType)
  } else if (args.remoteUrl) {
    await uploadAlbumPoster(wctx, args.rk, args.remoteUrl)
  } else {
    throw new Error('pushCoverToPlex: no image source provided')
  }
}
