  ...
  await job.run(
    ...
    async () => {
      ...
      for (const item of items) {
        ...
        try {
          const creditsCached = await isAlbumCreditsCached(item.ratingKey)
          const bioCached = options.creditsOnly ? true : await isAlbumBioCached(item)
          if (creditsCached && bioCached) {
            job.update({ done: job.progress.done + 1 })
            continue
          }

          let tracks: Awaited<ReturnType<typeof getAlbumDetail>>['tracks'] = []
          ...
          if (!creditsCached) {
            ...
            for (const win of BrowserWindow.getAllWindows()) {
              win.webContents.send('warmer:album-enriched', { ratingKey: item.ratingKey })
            }
          }

          if (!options.creditsOnly && !bioCached) {
            await getAlbumBio({
