...
// A cold MB release fetch (inc=recordings etc.) through musicbrainzngs has
// been measured at 20+ seconds before MB's cache warms; the retry is then
// sub-second. So the default is patient — long enough for a cold fetch, short
// enough that a genuinely wedged tagger (musicbrainzngs lock) still surfaces
// an error instead of hanging forever. Hot paths (artist click) pass a short
// timeout and fall through to an MB name search instead.
export const MB_RELEASE_TIMEOUT_MS = 45_000

export function getMbRelease(
  mbid: string,
  timeoutMs: number = MB_RELEASE_TIMEOUT_MS
): Promise<MbCanonicalRelease> {
  return getJson<MbCanonicalRelease>(`/mb/release/${encodeURIComponent(mbid)}`, timeoutMs)
}
