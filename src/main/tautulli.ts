...
export async function getTopChart(args: {
  ...
  const tally = new Map<
    string,
    { plays: number; title: string; subtitle: string | null; ratingKey: string | null; thumb: string | null }
  >()
  for (const rows of histories) {
    for (const row of rows) {
      ...
      if (cur) cur.plays += 1
      else tally.set(k.id, { plays: 1, title: k.title, subtitle: k.subtitle, ratingKey: k.ratingKey, thumb: k.thumb })
    }
  }
