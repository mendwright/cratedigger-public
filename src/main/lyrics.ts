...
// See genius.ts. Order: LRCLIB exact → LRCLIB fuzzy → Genius.
...
export async function getTrackLyrics(args: {
  ...
  const result = genius.value ?? lrclib.value ?? null

  // Cache positives always; cache a null only when both sources answered
  // cleanly (a genuine "no lyrics anywhere"), never when one errored.
  if (result !== null || (lrclib.ok && genius.ok)) {
