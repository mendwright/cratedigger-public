...
import { getSlskdPeerHistory } from './acquisitions.js'
import {
...
import { splitPath, guessArtistAlbum } from '../shared/slskd-folder.js'
...
async function req<T>(path: string, init?: RequestInit, timeoutMs = REQ_TIMEOUT_MS): Promise<T> {
  ...
  if (!configuredBase) throw new Error('slskd is not configured')
  ...
  try {
    const r = await fetchImpl(`${configuredBase}${path}`, {
      ...init,
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      throw new Error(`slskd ${path}: timed out after ${timeoutMs}ms`)
    }
    throw err
  } finally {
    clearTimeout(t)
...
export { splitPath, guessArtistAlbum } from '../shared/slskd-folder.js'
...
interface SlskdBrowseDirectory {
  name: string
  fileCount?: number
  files?: Array<{
    filename: string
...
interface SlskdBrowseResponse {
  directories?: SlskdBrowseDirectory[]
  lockedDirectories?: SlskdBrowseDirectory[]
}

/** Reduce a potentially enormous share tree to audio-bearing album folders. */
export function browseAlbumsFromResponse(
  response: SlskdBrowseResponse | SlskdBrowseDirectory[]
): { albums: SlskdBrowseAlbum[]; directoryCount: number; fileCount: number } {
  const directories = Array.isArray(response) ? response : (response.directories ?? [])
  const albums: SlskdBrowseAlbum[] = []
  let fileCount = 0
  for (const directory of directories) {
    const rawFiles = directory.files ?? []
    fileCount += directory.fileCount ?? rawFiles.length
    const sep = directory.name.includes('\\') ? '\\' : '/'
    const files = rawFiles
      .map((raw) => toFile({ ...raw, filename: `${directory.name}${sep}${raw.filename}` }))
      .filter((file): file is SlskdFile => file !== null)
      .sort((a, b) => a.filename.localeCompare(b.filename))
    if (files.length === 0) continue
    const parts = splitPath(files[0].filename)
    const guessed = guessArtistAlbum(parts)
    albums.push({
      folder: directory.name,
      name: splitPath(directory.name).at(-1) ?? directory.name,
      guessedArtist: guessed.artist,
      guessedAlbum: guessed.album,
      files,
      formatSummary: formatSummary(files)
    })
  }
  albums.sort((a, b) =>
    (a.guessedArtist ?? '').localeCompare(b.guessedArtist ?? '') ||
    (a.guessedAlbum ?? a.name).localeCompare(b.guessedAlbum ?? b.name)
  )
  return { albums, directoryCount: directories.length, fileCount }
}

export async function browseUser(username: string): Promise<SlskdUserBrowse> {
  const clean = username.trim()
  if (!clean) throw new Error('Soulseek username is required')
  // Large collections can take materially longer than an ordinary API call.
  const response = await req<SlskdBrowseResponse | SlskdBrowseDirectory[]>(
    `/api/v0/users/${encodeURIComponent(clean)}/browse`,
    undefined,
    120_000
  )
  return { username: clean, ...browseAlbumsFromResponse(response) }
}
...
export async function startSearch(searchText: string): Promise<{ id: string }> {
  const id = randomUUID()
...
// The Soulseek *server* silently drops searches containing certain overloaded
// terms — most notoriously "beatles", the most-searched artist in Soulseek
// history — relaying them to no peers, so the search times out with 0 responses
// even for ubiquitous albums (verified 2026-06: "abbey road" → 250 responses,
// "beatles abbey road" → 0). To stay resilient we try the full "artist album"
// query first, then fall back to album-only, then artist-only, breaking as soon
// as a query yields responses. MB reconcile + ranking always run on the
// original (artist, album), so dropping the artist from the *network query*
// never costs us the correct release/tracklist or artist-aware scoring.
export function buildSearchQueries(artist: string, album: string): string[] {
  ...
  add([a, b].filter(Boolean).join(' '))
  if (a && b) {
    add(b) // album-only — survives a blocked artist token
    add(a) // artist-only — last resort
  }
  return queries
}
...
export function rankResponses(
  ...
  expectedTitles: string[] = [],
  peerHistory: Record<string, SlskdPeerHistory> = {}
): SlskdAlbumCandidate[] {
  const q: QueryCtx = {
    artist,
    album,
  ...
  for (const b of buckets.values()) {
    ...
    candidates.push({
      ...
      matchedTracks,
      successfulAlbums: peerHistory[b.username]?.successfulAlbums ?? 0,
      failedAlbums: peerHistory[b.username]?.failedAlbums ?? 0,
      historicalAverageSpeed: peerHistory[b.username]?.averageSpeed ?? 0
    })
  }

  // Album acquisition order is intentionally legible: completeness first,
  // lossless fidelity next, then the peer's currently advertised speed.
  // Reputation is a final tie-break and a visible hint, not permission for a
  // slow familiar peer to outrank a much faster source.
  const completeness = (c: SlskdAlbumCandidate): number => expectedTitles.length > 0
    ? c.matchedTracks / expectedTitles.length
    : Math.min(1, c.files.length / 12)
  const losslessRatio = (c: SlskdAlbumCandidate): number =>
    c.files.filter((f) => f.extension === 'flac' || f.extension === 'wav').length / c.files.length
  candidates.sort((a, b) =>
    completeness(b) - completeness(a) ||
    losslessRatio(b) - losslessRatio(a) ||
...
export function groupCandidatesByAlbum(
  ...
  // Map insertion order follows the already-ranked candidates, so retaining
  // it keeps group cards under the same completeness/speed rule.
  return groups
}
...
// Start one slskd search for `searchText`, poll until it completes (or the
// timeout elapses), and return the materialised responses. Owns the search's
// full lifecycle including cleanup, so callers can fire several in sequence.
async function searchOnce(
  searchText: string,
  signal?: AbortSignal
): Promise<{ responses: SlskdSearchResponse[]; complete: boolean }> {
  const start = Date.now()
  const { id } = await startSearch(searchText)
  try {
    ...
    return { responses: lastResponses, complete }
  } finally {
    await stopSearch(id)
  }
}
...
export function searchAndWait(
  ...
  const run = async (): Promise<SlskdSearchResult> => {
    ...
    // if slskd finishes first (rare — slskd is slow). MB always reconciles on
    // the user's full (artist, album), regardless of which network query wins.
    const mbPromise = mbReleaseHint(artist, album, preferredReleaseMbid)
    // Try progressively looser network queries until one returns responses, so
    // a server-blocked artist token (e.g. "beatles") can't zero out an album
    // that peers actually have. See buildSearchQueries.
    let lastResponses: SlskdSearchResponse[] = []
    let complete = false
    for (const text of buildSearchQueries(artist, album)) {
      if (signal?.aborted) break
      const r = await searchOnce(text, signal)
      lastResponses = r.responses
      complete = r.complete
      if (lastResponses.length > 0) break
    }
    {
      const mb = await mbPromise
      let expectedTitles = mb?.titles ?? []
      const peerHistory = getSlskdPeerHistory()
      let candidates = rankResponses(artist, album, lastResponses, expectedTitles, peerHistory)
      ...
      if (
        ...
        if (better) {
          ...
          release = better.hint
          candidates = rankResponses(artist, album, lastResponses, expectedTitles, peerHistory)
        }
      }
      return {
        query: { artist, album },
