...
// Read a saved local cover back as bytes + content type so it can be POSTed
// straight to Plex — the local file is served over cratedigger-cover:// and
// Plex can't fetch that scheme, so pushing it to the server means uploading the
// raw image. Returns null when no local cover is saved for the album.
export async function readLocalCover(
  ...
  const file = entries.find((f) => f === key || f.startsWith(`${key}.`))
  if (!file) return null
  const ext = extname(file).toLowerCase()
  const bytes = await fs.readFile(join(dir, file))
  return { bytes, contentType: CONTENT_TYPE[ext] ?? 'image/jpeg' }
}
