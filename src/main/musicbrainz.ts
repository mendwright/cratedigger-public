...
export async function getAlbumCreditsCached(
  ...
  const finalize = async (c: AlbumCreditsWithContext): Promise<AlbumCredits> => {
    ...
    return {
      ...
      albumArtistMbid: c.albumArtistMbid ?? null,
...
// Supersession token for the recording-enrichment walk below: each new
// uncached credited-works request bumps it, and any older walk still in its
// (up to 30-lookup, ~35s paced) loop notices and bails out. Without this,
// clicking artist B queues B's every MB fetch behind the remainder of artist
// A's walk — the page "never loads". A superseded walk returns its partial
// list and skips the cache write; each recording lookup is individually
// cached, so a return visit resumes where it left off.
let creditedWalkSeq = 0
...
export async function getArtistCreditedWorks(mbid: string): Promise<CreditedWork[]> {
  ...
  if (!superseded) await creditedWorksCache.set(mbid, works)
  return works
}
...
// A release never changes which group it belongs to, and the genre indexer now
// asks for this on every album rather than only the ones MB had no release
// genres for — so caching it turns a second full crawl into a local read.
const releaseGroupOfReleaseCache = new JsonCache<{ id: string | null }>(
  'release-group-of-release-v1'
)
...
// Artist-level genres: the broadest tier, and often the only populated one —
// Dr. Feelgood's albums carry no genres on MB while the artist has "pub rock"
// with four votes. Cached per artist, so a library of 1,700 artists costs
// 1,700 requests once, not one per album.
const artistGenresCache = new JsonCache<string[]>('artist-genres-v1')

export async function getArtistGenres(mbid: string): Promise<string[]> {
  const cached = await artistGenresCache.get(mbid)
  if (cached !== undefined && cached !== null) return cached
  interface Res {
    genres?: MbGenreRow[]
  }
  const res = await mbFetch<Res>(`/artist/${mbid}?inc=genres&fmt=json`)
  const genres = readGenres(res.genres)
  await artistGenresCache.set(mbid, genres)
  return genres
}
...
export async function getReleaseGroupForRelease(
  ...
  const cached = await releaseGroupOfReleaseCache.get(releaseMbid)
  // Wrapped in an object so a cached "this release has no group" is
  // distinguishable from a cache miss and isn't re-fetched forever.
  if (cached !== undefined && cached !== null) return cached.id
  try {
    interface Res {
    ...
    const id = data['release-group']?.id ?? null
    await releaseGroupOfReleaseCache.set(releaseMbid, { id })
    return id
  } catch {
    // Deliberately not cached — a 503 during a crawl must not be remembered as
    // "this release has no group".
    return null
  }
}
