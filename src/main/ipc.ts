...
import { pushKioskTheme } from './kiosk-theme.js'
import {
  listMusicSections,
  listAlbums,
  listAllAlbums,
...
export function registerIpc(): void {
  ...
  register('plex:get-artist-mbid', async (_e, args) => {
    const ctx = await resolveServerContext(args.serverId)
    return await getArtistMbid(ctx, args.ratingKey)
  })
  ...
  register('tagger:get-release', async (_e, args) => {
    return await getMbRelease(args.mbid, args.timeoutMs)
  })
  if (!PUBLIC_BUILD) {
  ...
  register('settings:get', () => ({
    ...
    kioskUrl: getValue('kioskUrl'),
    continuationMode: getContinuationMode(),
    showNotificationsEnabled: getValue('showNotificationsEnabled'),
    autoWarmCredits: getValue('autoWarmCredits')
  }))

  register('kiosk:push-theme', async (_e, theme) => await pushKioskTheme(theme))
  ...
  register('settings:set', (_e, args) => {
    ...
    if (typeof args.kioskUrl === 'string') setValue('kioskUrl', args.kioskUrl.trim())
    ...
    if (typeof args.autoWarmCredits === 'boolean') setValue('autoWarmCredits', args.autoWarmCredits)
    return { ok: true as const }
  })
