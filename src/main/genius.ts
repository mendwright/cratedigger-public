...
// flow is: client_credentials → bearer token → /search for the song page URL →
...
// Returns `{ ok, value }`: ok=false means we couldn't determine (no creds, mint
// failed, or a transport error) and the caller must not cache the null;
// ok=true with value=null is a genuine "Genius has no lyrics for this".
export async function getGeniusLyrics(args: {
  artist: string
  title: string
}): Promise<{ ok: boolean; value: LyricsResult | null }> {
  let token = await getToken()
  if (!token) return { ok: false, value: null } // no creds, or mint failed (transient)
  let url: string | null
  try {
