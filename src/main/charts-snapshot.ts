...
const SNAPSHOT_URL =
  'https://github.com/mendwright/cratedigger-public/releases/download/charts-data/charts.json'
const TTL = 2 * 60_000
...
interface SnapshotEntry {
  ...
  thumb: string | null
  plays: number
  // Absent on snapshots published before time-based ranking; treated as 0.
  seconds?: number
}
interface Snapshot {
  generatedAt: number
...
export async function snapshotTop(args: {
  ...
  for (const uid of args.userIds) {
    for (const e of dimMap[String(uid)] || []) {
      ...
      if (cur) {
        cur.plays += e.plays
        cur.seconds = (cur.seconds ?? 0) + (e.seconds ?? 0)
      } else {
        tally.set(key, { ...e, seconds: e.seconds ?? 0 })
      }
    }
  }
  // Same ranking as the live path: time listened first, plays as tiebreak
  // (and as the whole order for a pre-seconds snapshot, where every seconds
  // value is 0).
  const entries = [...tally.values()]
    .sort((a, b) => (b.seconds ?? 0) - (a.seconds ?? 0) || b.plays - a.plays)
    .slice(0, limit)
    .map((e, i) => ({
      rank: i + 1,
      title: e.title,
