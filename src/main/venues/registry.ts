...
export const VENUES: Venue[] = [
  ...
  },
  {
    ...
    latitude: 37.7853,
    longitude: -122.4174,
    // GAMH tickets its calendar through the See Tickets WordPress plugin
    // (wl.seetickets.us white-label), not Ticketmaster. The TM Discovery feed
    // only carried the handful of bigger touring acts and silently dropped the
    // entire indie/experimental tail (e.g. Los Thuthanaka) — the See Tickets
    // calendar is a strict superset, so we read it directly. /calendar/ is the
    // full multi-month view; the homepage only shows a hero carousel.
